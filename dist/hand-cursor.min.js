...
.hc-root {
  ...
  --hc-tab-h: ${u.height}px;
  /* The card is square where the tab covers it and rounded everywhere else. */
  --hc-card-radius: ${g.card}px ${g.card}px 0 ${g.card}px;
  --hc-tab-radius: 0 ${g.card}px ${g.card}px 0;
}

.hc-root[data-state="live"] { --hc-tab-h: ${u.liveHeight}px; }

.hc-root[data-position$="-right"] {
  --hc-card-radius: ${g.card}px ${g.card}px ${g.card}px 0;
  --hc-tab-radius: ${g.card}px 0 0 ${g.card}px;
}
...
.hc-root[data-mini="true"][data-position$="-left"] .hc-dock {
  transform: translateX(calc(-1 * (var(--hc-margin) + ${p.panelWidth}px)));
}

.hc-root[data-mini="true"][data-position$="-right"] .hc-dock {
  transform: translateX(calc(var(--hc-margin) + ${p.panelWidth}px));
}
...
.hc-stage {
  ...
  flex: 0 0 auto;
  width: ${p.illoWidth}px;
  height: ${p.illoHeight}px;
  display: flex;
  align-items: center;
...
.hc-root[data-position$="-right"] .hc-scrim { background: ${st(270)}; }
...
.hc-copy {
  ...
  width: 100%;
  text-align: center;
  font-size: ${y.body.size}px;
  font-weight: ${y.body.weight};
  line-height: ${y.body.leading}px;
  letter-spacing: -0.01em;
  color: ${l.textSecondary};
}

.hc-root[data-state="error"] .hc-copy { color: ${l.red}; }
...
.hc-cta {
  ...
  justify-content: center;
  gap: ${p.ctaGap}px;
  height: ${p.ctaHeight}px;
  padding: 0 ${p.ctaPadX}px;
  border-radius: ${g.button}px;
  background: ${l.green};
  color: ${l.white};
  font-family: inherit;
  font-size: ${y.label.size}px;
  font-weight: ${y.label.weight};
  line-height: ${y.label.leading}px;
  letter-spacing: -0.01em;
  cursor: pointer;
...
.hc-cta:hover,
.hc-cta.hc-hover { background: ${l.darkGreen}; }
.hc-cta:active { transform: scale(0.97); }
.hc-cta[disabled] { opacity: 0.7; cursor: default; }

.hc-cta svg { width: ${p.iconSize}px; height: ${p.iconSize}px; flex: none; }
...
.hc-corner {
  ...
  justify-content: center;
  cursor: pointer;
  width: ${p.cornerButton}px;
  height: ${p.cornerButton}px;
  border-radius: ${g.button}px;
  background: ${l.red};
  color: ${l.white};
...
.hc-corner svg { width: ${p.iconSize}px; height: ${p.iconSize}px; }
...
.hc-corner--tl { top: ${p.cornerInset}px; left: ${p.cornerInset}px; }
...
.hc-tab {
  ...
  padding: ${u.pad}px ${(u.width-u.iconWidth)/2}px;
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
...
.hc-tab > svg {
  display: block;
  width: ${u.iconWidth}px;
  height: ${u.iconHeight}px;
  flex: none;
  /*
...
.hc-root[data-mini="true"] .hc-corner {
  visibility: hidden;
  transition: visibility 0s linear ${Z}ms;
}
...
.hc-cursor {
  ...
  transition: opacity ${et}ms linear;
}
...
.hc-debug-row b.is-warn { color: ${l.yellow}; }
      ...
      <div class="hc-dock">
        ...
        <div class="hc-panel" role="region" aria-label="Hand tracking trackpad">
          <div class="hc-stage">
            ...
            <div class="hc-illo"><img class="hc-illo-img" src="${ft}" alt="" width="${p.illoWidth}" height="${p.illoHeight}" draggable="false" /></div>
          </div>
          <p class="hc-copy"></p>
        ...
        <button class="hc-tab" type="button">
          <span class="hc-tab-notch" aria-hidden="true">${k.notch}</span>
          <span class="hc-tab-dot" aria-hidden="true"></span>
          ${k.chevron}
        </button>
      </div>
      <p class="hc-sr" role="status" aria-live="polite"></p>
    `;let i=s=>this.root.querySelector(s);this.panel=i(".hc-panel"),this.video=i(".hc-video"),this.canvas=i(".hc-overlay"),this.copy=i(".hc-copy"),this.cta=i(".hc-cta"),this.cornerLeft=i(".hc-corner--tl"),this.tab=i(".hc-tab"),this.status=i(".hc-sr"),this.ctx=this.canvas.getContext("2d"),this.canvasSize={width:0,height:0,dpr:0},this.cornerLeft.innerHTML=k.videocamOff,this.cornerLeft.title=t.strings.disable,this.cornerLeft.setAttribute("aria-label",t.strings.disable),this.cta.addEventListener("click",s=>e.onToggleCamera(s)),this.tab.addEventListener("click",()=>e.onToggleSize()),this.cornerLeft.addEventListener("click",()=>e.onStop()),this.render()}mount(t){t.appendChild(this.root)}setState(t,e){this.state=t,this.root.dataset.state=t,this.message=e,this.render()}setMini(t){this.mini=t,this.root.dataset.mini=String(t),this.render()}render(){let t=this.options.strings,e=this.state==="loading",i=this.state==="live";this.copy.textContent=this.state==="error"?this.message||t.failed:t.intro;let s=e?t.starting:this.state==="error"?t.retry:t.enable;this.cta.innerHTML=(e?'<span class="hc-spinner"></span>':k.videocam)+`<span>${s}</span>`,this.cta.disabled=e;let o=this.mini?t.expand:t.minimize;this.tab.title=o,this.tab.setAttribute("aria-label",o),this.status.textContent=i?"Hand tracking is on.":this.state==="error"?this.message||t.failed:""}attachStream(t){return this.video.srcObject=t,this.video.play().catch(()=>{})}detachStream(){this.video.srcObject=null,this.clearOverlay()}syncCanvas(){let t=Math.min(window.devicePixelRatio||1,2),e=this.canvas.clientWidth,i=this.canvas.clientHeight;if(!e||!i)return null;let s=this.canvasSize;return(s.width!==e||s.height!==i||s.dpr!==t)&&(this.canvas.width=Math.round(e*t),this.canvas.height=Math.round(i*t),this.canvasSize={width:e,height:i,dpr:t}),{width:e,height:i,dpr:t}}clearOverlay(){this.canvas.width&&(this.ctx.setTransform(1,0,0,1,0,0),this.ctx.clearRect(0,0,this.canvas.width,this.canvas.height))}drawHand(t,e){let i=this.syncCanvas();if(!i)return;let{videoWidth:s,videoHeight:o}=this.video;if(this.ctx.setTransform(i.dpr,0,0,i.dpr,0,0),this.ctx.clearRect(0,0,i.width,i.height),!t||!s||!o)return;let n=Math.max(i.width/s,i.height/o),h=s*n,c=o*n,a=(i.width-h)/2,d=(i.height-c)/2,f=t.map(v=>({x:a+(1-v.x)*h,y:d+v.y*c}));ut(this.ctx,f,{pinching:e})}destroy(){this.detachStream(),this.root.remove()}};var Ht=(r,t,e)=>r<t?t:r>e?e:r,z=class{constructor(t){this.options=t,this.el=document.createElement("div"),this.el.className="hc-cursor",this.el.setAttribute("aria-hidden","true"),this.el.innerHTML=nt,this.x=0,this.y=0,this.angle=0,this.scale=t.cursor.scale,this.visible=!1,this.pressed=!1,this.lastMoveAt=0,this.reducedMotion=typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches}mount(t){t.appendChild(this.el)}setVisible(t){this.visible!==t&&(this.visible=t,this.el.dataset.visible=String(t))}setPressed(t){this.pressed=t}update(t,e,i,s,o){this.x=t,this.y=e;let n=this.options.rotation;if(n.enabled&&!this.reducedMotion){let c=Math.abs(i)<n.minSpeed?0:Ht(i*n.gain,-n.maxAngle,n.maxAngle);this.angle+=(c-this.angle)*n.smoothing}else this.angle=0;let h=this.pressed?this.options.cursor.pressScale:this.options.cursor.scale;this.scale+=(h-this.scale)*.35,this.el.style.transform=`translate3d(${t.toFixed(2)}px, ${e.toFixed(2)}px, 0) rotate(${this.angle.toFixed(2)}deg) scale(${this.scale.toFixed(3)})`}reset(){this.angle=0,this.scale=this.options.cursor.scale,this.pressed=!1}destroy(){this.el.remove()}};function I(r,t,e){return{bubbles:!0,cancelable:!0,composed:!0,view:window,clientX:r,clientY:t,screenX:(window.screenX||0)+r,screenY:(window.screenY||0)+t,...e}}var Gt={pointerId:1,pointerType:"touch",isPrimary:!0,width:1,height:1};function m(r,t,e,i,s){r.dispatchEvent(new PointerEvent(t,I(e,i,{...Gt,...s})))}function b(r,t,e,i,s){r.dispatchEvent(new MouseEvent(t,I(e,i,s)))}function x(r,t,e,i,s){return r.dispatchEvent(new DragEvent(t,I(e,i,{dataTransfer:s})))}var gt=/(auto|scroll|overlay)/;function mt(r){let t=getComputedStyle(r);return gt.test(t.overflowY)&&r.scrollHeight-r.clientHeight>1||gt.test(t.overflowX)&&r.scrollWidth-r.clientWidth>1}function Ut(r,t){let e=r;for(;e.parentElement;){let i=e.parentElement;if(getComputedStyle(i).cursor!==t)return e;if(i===document.body||i===document.documentElement)return null;e=i}return null}function R(r,t){let{selector:e,cursors:i,touchAction:s}=t,o=new Set(i),n=r;for(;n&&n.nodeType===1;){if(n.getAttribute?.("draggable")==="true")return{node:n,html5:!0};if(e&&n.matches?.(e))return{node:n,html5:!1};if(!(n===document.body||n===document.documentElement)&&!mt(n)){let a=getComputedStyle(n);if(o.has(a.cursor)){let d=Ut(n,a.cursor);if(d&&!mt(d))return{node:d,html5:!1}}if(s&&a.touchAction==="none")return{node:n,html5:!1}}n=n.parentElement||n.getRootNode()?.host||null}return null}var O=class{constructor(t,e,i,s,o){this.node=t.node,this.html5=t.html5&&o,this.started=!1,this.over=null,this.canDrop=!1,this.dataTransfer=null,this.pressed=e||this.node,m(this.pressed,"pointerdown",i,s,{buttons:1,button:0}),b(this.pressed,"mousedown",i,s,{buttons:1,button:0})}start(t,e){if(!this.started&&(this.started=!0,!!this.html5)){try{this.dataTransfer=new DataTransfer}catch{return}this.dataTransfer.effectAllowed="all",x(this.node,"dragstart",t,e,this.dataTransfer)}}move(t,e,i){let s=i||(this.pressed.isConnected?this.pressed:document);m(s,"pointermove",t,e,{buttons:1}),b(s,"mousemove",t,e,{buttons:1}),this.dataTransfer&&(x(this.node,"drag",t,e,this.dataTransfer),i!==this.over&&(this.over&&x(this.over,"dragleave",t,e,this.dataTransfer),i&&x(i,"dragenter",t,e,this.dataTransfer),this.over=i),this.canDrop=i?!x(i,"dragover",t,e,this.dataTransfer):!1)}clicks(t,e,i){if(i&&(this.pressed===i||this.pressed.contains(i)||i.contains(this.pressed)))return!0;let s=this.pressed.getBoundingClientRect?.();return!s||s.width===0&&s.height===0?!0:t>=s.left&&t<=s.right&&e>=s.top&&e<=s.bottom}end(t,e,i){let s=i||(this.pressed.isConnected?this.pressed:document);m(s,"pointerup",t,e,{buttons:0,button:0}),b(s,"mouseup",t,e,{buttons:0,button:0});let o=!!(this.dataTransfer&&this.over&&this.canDrop);return this.dataTransfer&&(o&&x(this.over,"drop",t,e,this.dataTransfer),x(this.node,"dragend",t,e,this.dataTransfer)),o}cancel(t,e){let i=this.pressed.isConnected?this.pressed:document;m(i,"pointercancel",t,e,{buttons:0}),this.dataTransfer&&(this.over&&x(this.over,"dragleave",t,e,this.dataTransfer),x(this.node,"dragend",t,e,this.dataTransfer))}};var bt=/(auto|scroll|overlay)/;function yt(r){let t=r;for(;t&&t!==document.documentElement&&t!==document.body;){if(t.nodeType===1){let i=getComputedStyle(t),s=bt.test(i.overflowY)&&t.scrollHeight-t.clientHeight>1,o=bt.test(i.overflowX)&&t.scrollWidth-t.clientWidth>1;if(s||o)return{node:t,canX:o,canY:s}}t=t.parentElement||t.getRootNode()?.host||null}let e=document.scrollingElement||document.documentElement;return{node:e,canX:e.scrollWidth-e.clientWidth>1,canY:e.scrollHeight-e.clientHeight>1}}function jt(r){let t=r===document.scrollingElement?document.documentElement:r,e=t.style.scrollBehavior;return t.style.setProperty("scroll-behavior","auto","important"),()=>{e?t.style.setProperty("scroll-behavior",e,""):t.style.removeProperty("scroll-behavior")}}function Kt(r){let t=r===document.scrollingElement?document.documentElement:r;return getComputedStyle(t).scrollBehavior==="smooth"}function L(r){let t=window.devicePixelRatio||1;return Math.round(r*t)/t}function vt(r){return{x:Math.max(0,r.scrollWidth-r.clientWidth),y:Math.max(0,r.scrollHeight-r.clientHeight)}}function wt(r){return-1/(60*Math.log(r))}function xt(r,t,e){return r*wt(t)*e}var Bt=.05,Wt=.15,P=class{constructor(t,e){this.options=t,this.debug=e,this.target=null,this.askedX=0,this.askedY=0,this.appliedX=0,this.appliedY=0,this.path=[],this.interval=0,this.head=0,this.positionX=0,this.positionY=0,this.live=!1,this.velocityX=0,this.velocityY=0,this.flinging=!1,this.frame=null,this.lastFrameAt=0,this.restoreBehavior=null,this.lastRetargetAt=0,this.tick=this.tick.bind(this)}resetPath(){this.askedX=0,this.askedY=0,this.appliedX=0,this.appliedY=0,this.path=[],this.interval=0,this.head=0,this.live=!1}setTarget(t){t!==this.target&&(this.resetPath(),this.releaseBehavior()),this.target=t,t&&(this.claimPosition(),this.debug?.recordTarget(t.node,Kt(t.node)),this.debug?.beginTrace(t.node),this.lastRetargetAt=0,(this.options.drag.mode==="write"||this.options.drag.mode==="hybrid")&&(this.restoreBehavior||(this.restoreBehavior=jt(t.node))))}releaseBehavior(){this.restoreBehavior?.(),this.restoreBehavior=null}claimPosition(){this.target&&(this.positionX=this.target.node.scrollLeft,this.positionY=this.target.node.scrollTop)}applyScroll(t,e){let{node:i,canX:s,canY:o}=this.target,n=vt(i);o&&(this.positionY=Math.max(0,Math.min(n.y,this.positionY-e))),s&&(this.positionX=Math.max(0,Math.min(n.x,this.positionX-t)));let h=L(this.positionY),c=L(this.positionX);try{i.scrollTo({top:h,left:c,behavior:"instant"})}catch{i.scrollTop=h,i.scrollLeft=c}this.debug&&this.debug.recordCommit(h,o?i.scrollTop:i.scrollLeft)}push(t,e,i){if(!this.target)return;this.flinging=!1,this.live=!0;let s=this.path.at(-1);if(s){let o=i-s.t;o>=8&&o<=250&&(this.interval=this.interval?this.interval*.7+o*.3:o)}if(this.path.length===0){let o=this.options.drag.resampleMax;this.path.push({t:i-o,x:this.askedX,y:this.askedY})}for(this.askedX+=t,this.askedY+=e,this.path.push({t:i,x:this.askedX,y:this.askedY});this.path.length>2&&i-this.path[0].t>1e3;)this.path.shift();this.options.drag.mode==="native"?this.retarget():this.start()}delay(){let{resample:t,resampleMin:e,resampleMax:i}=this.options.drag;return this.interval?Math.max(e,Math.min(i,this.interval*t)):i}advanceHead(t,e){let i=t-this.delay();if(!this.head)return this.head=i,this.head;this.head+=e;let s=e*Bt,o=(i-this.head)*Wt;return this.head+=Math.max(-s,Math.min(s,o)),this.head}positionAt(t){let e=this.path;if(e.length===0)return{x:this.appliedX,y:this.appliedY};let i=e[0];if(t<=i.t)return{x:i.x,y:i.y};let s=e.at(-1);if(t>=s.t){let d=e.at(-2),f=d?s.t-d.t:0;if(!this.live||!d||f<=0)return{x:s.x,y:s.y};let v=Math.min(t-s.t,this.options.drag.resampleMax)/f;return{x:s.x+(s.x-d.x)*v,y:s.y+(s.y-d.y)*v}}let o=e.length-1;for(;o>0&&e[o-1].t>t;)o-=1;let n=e[o-1],h=e[o],c=h.t-n.t,a=c>0?(t-n.t)/c:1;return{x:n.x+(h.x-n.x)*a,y:n.y+(h.y-n.y)*a}}retarget(t=!1,e="smooth"){let i=performance.now(),{retargetMs:s}=this.options.drag;if(!t&&i-this.lastRetargetAt<s)return!0;this.lastRetargetAt=i;let{node:o,canX:n,canY:h}=this.target,c=vt(o);h&&(this.positionY=Math.max(0,Math.min(c.y,this.positionY-this.remainingY))),n&&(this.positionX=Math.max(0,Math.min(c.x,this.positionX-this.remainingX)));let a=L(this.positionY),d=L(this.positionX);try{o.scrollTo({top:a,left:d,behavior:e})}catch{return!1}return this.appliedX=this.askedX,this.appliedY=this.askedY,this.debug?.recordScroll(0),!0}fling(t,e){let{minVelocity:i,maxVelocity:s,mode:o,friction:n,flingScale:h}=this.options.drag,c=D=>Math.max(-s,Math.min(s,D)),a=c(t),d=c(e),f=Math.hypot(a,d)<i;if(this.live=!1,(o==="native"||o==="hybrid")&&(f||(this.askedX+=xt(a,n,h),this.askedY+=xt(d,n,h)),this.releaseBehavior(),this.retarget(!0,"smooth"))){this.stop();return}if(f)return;let v=wt(n);this.velocityX=a+this.remainingX/v,this.velocityY=d+this.remainingY/v,this.appliedX=this.askedX,this.appliedY=this.askedY,this.flinging=!0,this.start()}start(){this.frame===null&&(this.lastFrameAt=0,this.frame=requestAnimationFrame(this.tick))}stop(){this.frame!==null&&(cancelAnimationFrame(this.frame),this.frame=null),this.flinging=!1,this.resetPath(),this.velocityX=0,this.velocityY=0,this.releaseBehavior()}tick(t){if(this.frame=null,!this.target)return;let e=this.lastFrameAt?Math.min(t-this.lastFrameAt,50):1e3/60,i=e/1e3;this.lastFrameAt=t;let s=0,o=0;if(this.flinging){let{friction:h,minVelocity:c}=this.options.drag,a=h**(i*60);this.velocityX*=a,this.velocityY*=a,s=this.velocityX*i,o=this.velocityY*i,Math.hypot(this.velocityX,this.velocityY)<c&&(this.flinging=!1)}else if(this.options.drag.follow>=1)s=this.remainingX,o=this.remainingY,this.appliedX=this.askedX,this.appliedY=this.askedY;else{let h=this.positionAt(this.advanceHead(t,e));s=h.x-this.appliedX,o=h.y-this.appliedY,this.appliedX=h.x,this.appliedY=h.y}(s||o)&&(this.applyScroll(s,o),this.debug?.recordScroll(o||s));let n=Math.abs(this.remainingX)<.001&&Math.abs(this.remainingY)<.001;!this.flinging&&n?(this.releaseBehavior(),this.debug?.endTrace()):this.frame=requestAnimationFrame(this.tick)}get remainingX(){return this.askedX-this.appliedX}get remainingY(){return this.askedY-this.appliedY}};var Dt="a[href], button, input, select, textarea, summary, [tabindex]",Vt=1e3;function Nt(r,t){let e=document.elementFromPoint(r,t);for(;e?.shadowRoot;){let i=e.shadowRoot.elementFromPoint(r,t);if(!i||i===e)break;e=i}return e}var X=class{constructor(t,{ui:e,onTap:i,onGrab:s,hover:o,debug:n}={}){this.options=t,this.ui=e,this.onTap=i,this.onGrab=s,this.hoverStyles=o,this.hovered=null,this.pressing=!1,this.dragging=!1,this.origin=null,this.last=null,this.samples=[],this.scrollTarget=null,this.scrolled=!1,this.grabTarget=null,this.grab=null,this.debug=n,this.scroller=new P(t,n)}resolve(t,e){let i=Nt(t,e);return{el:i,internal:!!this.ui?.contains(i)}}move(t,e){let{el:i,internal:s}=this.resolve(t,e);if(this.ui?.hover(s?i:null,t,e),s||!i){this.leaveHovered(t,e);return}i!==this.hovered&&(this.leaveHovered(t,e),this.hovered=i,this.hoverStyles?.set(i),m(i,"pointerover",t,e,{buttons:this.pressing?1:0}),b(i,"mouseover",t,e,{buttons:this.pressing?1:0}),i.dispatchEvent(new MouseEvent("mouseenter",{...I(t,e),bubbles:!1,cancelable:!1}))),m(i,"pointermove",t,e,{buttons:this.pressing?1:0}),b(i,"mousemove",t,e,{buttons:this.pressing?1:0})}leaveHovered(t,e){let i=this.hovered;i&&(this.hovered=null,this.hoverStyles?.clear(),m(i,"pointerout",t,e,{buttons:0}),b(i,"mouseout",t,e,{buttons:0}),i.dispatchEvent(new MouseEvent("mouseleave",{...I(t,e),bubbles:!1,cancelable:!1})))}press(t,e,i){this.scroller.stop();let{el:s,internal:o}=this.resolve(t,e);this.pressing=!0,this.dragging=!1,this.origin={x:t,y:e,t:i,el:s,internal:o},this.last={x:t,y:e,t:i},this.samples=[{x:t,y:e,t:i}];let n=this.options.grab;this.grabTarget=n.enabled&&s&&!o?R(s,n):null,this.scrolled=!1;let h=!!this.grabTarget&&n.holdDelay===0;this.scrollTarget=o||!s||h?null:yt(s),this.scroller.setTarget(this.scrollTarget),h&&this.beginGrab(t,e,s)}pastDragGate(t,e,i,s){let{holdDelay:o,holdEscape:n}=this.options.drag,h=Math.hypot(t-this.origin.x,e-this.origin.y);return h<n&&i-this.origin.t<o?!1:h>=s}drag(t,e,i){if(!this.pressing)return;let s=t-this.last.x,o=e-this.last.y;this.last={x:t,y:e,t:i},this.samples.push({x:t,y:e,t:i});let n=this.options.drag.velocityWindow*2;for(;this.samples.length>2&&i-this.samples[0].t>n;)this.samples.shift();if(this.grab){!this.dragging&&i-this.origin.t>=this.options.grab.tapDuration&&(this.dragging=!0,this.grab.start(t,e)),this.dragElement(t,e);return}if(!this.dragging){if(!this.pastDragGate(t,e,i,this.options.drag.threshold))return;this.dragging=!0,this.leaveHovered(t,e)}if(this.grabTarget&&!this.scrolled&&i-this.origin.t>=this.options.grab.holdDelay){this.beginGrab(t,e,this.resolve(t,e).el),this.grab.start(t,e),this.dragElement(t,e);return}this.scrolled=!0,this.scroller.push(s,o,i)}beginGrab(t,e,i){this.scroller.stop(),this.scrollTarget=null,this.grab=new O(this.grabTarget,i,t,e,this.options.grab.html5),this.onGrab?.({type:"start",target:this.grab.node,x:t,y:e})}dragElement(t,e){let{el:i,internal:s}=this.resolve(t,e);this.grab.move(t,e,s?null:i)}releaseVelocity(){let t=this.samples.at(-1);if(!t)return{x:0,y:0};let e=t.t-this.options.drag.velocityWindow,i=this.samples.length-1;for(;i>0&&this.samples[i-1].t>=e;)i-=1;let s=this.samples[Math.max(i-1,0)],o=(t.t-s.t)/Vt;return o<=0?{x:0,y:0}:{x:(t.x-s.x)/o,y:(t.y-s.y)/o}}release(t,e,i){if(!this.pressing)return;let s=this.origin;if(this.pressing=!1,this.origin=null,this.grab){let o=this.grab,n=this.dragging||i-s.t>=this.options.grab.tapDuration;this.grab=null,this.grabTarget=null,this.dragging=!1;let{el:h,internal:c}=this.resolve(t,e),a=o.end(t,e,c?null:h);if(this.debug?.recordGesture({ms:i-s.t,travel:Math.hypot(t-s.x,e-s.y),verdict:n?"drag":"press"}),!n&&o.clicks(t,e,c?null:h)){let d=()=>this.click(o.pressed,s.x,s.y),{clickDelay:f}=this.options.grab;f>0?setTimeout(d,f):d()}this.onGrab?.({type:"end",target:o.node,dropped:a,x:t,y:e});return}if(this.grabTarget=null,this.dragging){this.dragging=!1;let o=this.releaseVelocity();this.scroller.fling(o.x,o.y);return}this.isTap(s,t,e,i)&&this.tap(s.x,s.y)}isTap(t,e,i,s){let{maxDuration:o,maxTravel:n}=this.options.tap;return Math.hypot(e-t.x,i-t.y)<=n&&s-t.t<=o}click(t,e,i){let s=t.closest?.(Dt);if(s)try{s.focus({preventScroll:!0})}catch{}else document.activeElement&&document.activeElement!==document.body&&document.activeElement.blur?.();b(t,"click",e,i,{buttons:0,button:0,detail:1}),this.onTap?.({x:e,y:i,target:t,internal:!1})}tap(t,e){let{el:i,internal:s}=this.resolve(t,e);if(i){if(s){this.ui.tap(i,t,e),this.onTap?.({x:t,y:e,target:i,internal:!0});return}m(i,"pointerdown",t,e,{buttons:1,button:0}),b(i,"mousedown",t,e,{buttons:1,button:0}),m(i,"pointerup",t,e,{buttons:0,button:0}),b(i,"mouseup",t,e,{buttons:0,button:0}),this.click(i,t,e)}}cancel(t=0,e=0){this.scroller.stop(),this.grab&&(this.grab.cancel(t,e),this.onGrab?.({type:"end",target:this.grab.node,dropped:!1,x:t,y:e}),this.grab=null),this.grabTarget=null,this.pressing=!1,this.dragging=!1,this.origin=null,this.scrollTarget=null,this.ui?.hover(null,t,e),this.leaveHovered(t,e)}destroy(){this.cancel()}};function At(r,t){return 1/(1+1/(2*Math.PI*r)/t)}var Q=class{constructor(){this.value=null}filter(t,e){return this.value=this.value===null?t:e*t+(1-e)*this.value,this.value}reset(){this.value=null}},H=class{constructor({minCutoff:t=1,beta:e=0,dCutoff:i=1}={}){this.minCutoff=t,this.beta=e,this.dCutoff=i,this.x=new Q,this.dx=new Q}filter(t,e){e>0||(e=1/60);let i=this.x.value,s=i===null?0:(t-i)/e,o=this.dx.filter(s,At(this.dCutoff,e)),n=this.minCutoff+this.beta*Math.abs(o);return this.x.filter(t,At(n,e))}reset(){this.x.reset(),this.dx.reset()}},Y=class{constructor(t){this.fx=new H(t),this.fy=new H(t)}filter(t,e,i){return{x:this.fx.filter(t,i),y:this.fy.filter(e,i)}}reset(){this.fx.reset(),this.fy.reset()}};var G="data-hc-hover",Zt=r=>r.replace(/:hover\b/g,`[${G}]`);function J(r,t){for(let e of r)if(e.selectorText)e.selectorText.includes(":hover")&&t.push(`${Zt(e.selectorText)}{${e.style.cssText}}`);else if(e.media){let i=[];J(e.cssRules,i),i.length&&t.push(`@media ${e.conditionText}{${i.join("")}}`)}else if(e.conditionText&&e.cssRules){let i=[];J(e.cssRules,i),i.length&&t.push(`@supports ${e.conditionText}{${i.join("")}}`)}}var U=class{constructor(){this.style=null,this.chain=[],this.sheetCount=-1,this.warned=!1,this.observer=null}build(){let t=[],e=0;for(let i of document.styleSheets)if(i.ownerNode!==this.style)try{J(i.cssRules,t)}catch{e+=1}e&&!this.warned&&(this.warned=!0,console.info(`[hand-cursor] ${e} cross-origin stylesheet(s) could not be read, so their :hover styles will not respond to the hand cursor.`)),this.style||(this.style=document.createElement("style"),this.style.setAttribute("data-hand-cursor-hover","")),this.style.textContent=t.join(`
`),document.head.appendChild(this.style),this.sheetCount=document.styleSheets.length,this.observer||this.watch()}watch(){let t=0;this.observer=new MutationObserver(()=>{document.styleSheets.length!==this.sheetCount&&(clearTimeout(t),t=setTimeout(()=>this.build(),250))}),this.observer.observe(document.documentElement,{childList:!0,subtree:!0})}set(t){if(this.chain[0]===t)return;let e=[];for(let i=t;i&&i.nodeType===1;i=i.parentElement)e.push(i);for(let i of this.chain)e.includes(i)||i.removeAttribute(G);for(let i of e)i.setAttribute(G,"");this.chain=e}clear(){for(let t of this.chain)t.removeAttribute(G);this.chain=[]}destroy(){this.clear(),this.observer?.disconnect(),this.observer=null,this.style?.remove(),this.style=null}};var Et=r=>r<0?0:r>1?1:r,j=class{constructor(t,{ui:e,onEvent:i,debug:s}={}){this.options=t,this.onEvent=i,this.debug=s,this.cursor=new z(t),this.hoverStyles=t.emulateHover?new U:null,this.touch=new X(t,{ui:e,debug:s,hover:this.hoverStyles,onTap:o=>this.emit("tap",o),onGrab:({type:o,...n})=>this.emit(o==="start"?"grab":"drop",n)}),this.filter=new Y(t.smoothing),this.position=null,this.velocity={x:0,y:0},this.pinching=!1,this.lastFrameAt=0}mount(t){this.cursor.mount(t)}emit(t,e){this.onEvent?.(t,e)}consume(t,e,i=1){this.debug?.recordTracked();let s=this.lastFrameAt?(e-this.lastFrameAt)/1e3:1/60;this.lastFrameAt=e;let o=lt(t),n=this.options.region,h=Et((1-o.x-n.x)/(1-2*n.x)),c=Et((o.y-n.y)/(1-2*n.y)),a=this.filter.filter(h*window.innerWidth,c*window.innerHeight,s),d=this.position;this.position=a,this.velocity=d?{x:a.x-d.x,y:a.y-d.y}:{x:0,y:0},this.cursor.setVisible(!0),this.cursor.update(a.x,a.y,this.velocity.x,this.velocity.y,e);let f=ct(t,i),{on:v,off:D}=this.options.pinch,_=this.pinching?f<D:f<v;_&&!this.pinching?(this.pinching=!0,this.cursor.setPressed(!0),this.touch.press(a.x,a.y,e),this.emit("press",{x:a.x,y:a.y})):!_&&this.pinching&&(this.pinching=!1,this.cursor.setPressed(!1),this.touch.release(a.x,a.y,e),this.emit("release",{x:a.x,y:a.y})),this.pinching?this.touch.drag(a.x,a.y,e):this.touch.move(a.x,a.y);let tt={x:a.x,y:a.y,pinching:this.pinching,ratio:f};return this.emit("move",tt),tt}release(){this.position?this.touch.cancel(this.position.x,this.position.y):this.touch.cancel(),this.pinching=!1,this.position=null,this.lastFrameAt=0,this.filter.reset(),this.cursor.setPressed(!1),this.cursor.setVisible(!1)}reset(){this.filter.reset(),this.cursor.reset(),this.lastFrameAt=0}setSmoothing(t){this.options.smoothing={...this.options.smoothing,...t},this.filter=new Y(this.options.smoothing),this.lastFrameAt=0}prepareHoverStyles(){this.hoverStyles?.build()}destroy(){this.touch.destroy(),this.cursor.destroy(),this.hoverStyles?.destroy()}};var C=class{constructor(){this.values=[]}push(t){this.values.push(t),this.values.length>60&&this.values.shift()}get mean(){return this.values.length?this.values.reduce((t,e)=>t+e,0)/this.values.length:0}percentile(t){if(!this.values.length)return 0;let e=[...this.values].sort((i,s)=>i-s);return e[Math.min(e.length-1,Math.floor(e.length*t))]}get max(){return this.values.length?Math.max(...this.values):0}},qt=[["paint","repaints per second \u2014 the rate the screen can actually update"],["track","landmark frames per second"],["model","milliseconds per inference, mean / worst"],["frame","milliseconds between repaints, median / 95th"],["worst","longest gap between repaints"],["blocked","share of repaints later than 32ms"],["scroll","scroll writes per second, and mean step"],["commit","how far the browser\u2019s reported scroll offset trails what was just written, and how often it had not moved at all. Anything but 0 means this platform commits scrolls asynchronously"],["target","what is being scrolled, and whether its CSS asked for smooth"],["gesture","the last press: how long the pinch was actually held, how far the hand travelled, and what it was read as. The duration is the one to check against grab.tapDuration when presses are landing as drags"]],K=class{constructor(){this.inference=new C,this.frameGaps=new C,this.scrollSteps=new C,this.commitLag=new C,this.staleReads=0,this.commitReads=0,this.lastReported=null,this.paintCount=0,this.trackCount=0,this.scrollCount=0,this.blockedCount=0,this.lastPaintAt=0,this.lastReportAt=0,this.running=!1,this.targetLabel="",this.targetSmooth=!1,this.gestureLabel="",this.trace=[],this.traceNode=null,this.lastScrollTop=null,this.tracing=!1,this.el=document.createElement("div"),this.el.className="hc-debug",this.el.innerHTML=qt.map(([t,e])=>`<div class="hc-debug-row" title="${e}"><span>${t}</span><b data-k="${t}">\u2014</b></div>`).join("")+'<div class="hc-debug-trace"><span>per-frame scroll</span><code data-k="trace">drag the page to record</code></div>',this.tick=this.tick.bind(this)}mount(t){t.appendChild(this.el),this.start()}start(){this.running||(this.running=!0,this.lastPaintAt=0,this.lastReportAt=0,requestAnimationFrame(this.tick))}stop(){this.running=!1}recordInference(t){this.inference.push(t)}recordTracked(){this.trackCount+=1}recordScroll(t){this.scrollCount+=1,this.scrollSteps.push(Math.abs(t))}recordCommit(t,e){this.commitReads+=1,this.commitLag.push(Math.abs(t-e)),this.lastReported!==null&&e===this.lastReported&&(this.staleReads+=1),this.lastReported=e}beginTrace(t){this.traceNode=t,this.trace=[],this.lastScrollTop=null,this.tracing=!0}endTrace(){this.tracing=!1}recordGesture({ms:t,travel:e,verdict:i}){this.gestureLabel=`${Math.round(t)}ms ${Math.round(e)}px ${i}`}recordTarget(t,e){let i=t===document.scrollingElement||t===document.documentElement?"page":t.tagName.toLowerCase()+(t.id?`#${t.id}`:"");this.targetLabel=`${i} ${e?"SMOOTH":"auto"}`,this.targetSmooth=e}tick(t){if(!this.running)return;if(requestAnimationFrame(this.tick),this.lastPaintAt){let n=t-this.lastPaintAt;this.frameGaps.push(n),this.paintCount+=1,n>32&&(this.blockedCount+=1)}if(this.lastPaintAt=t,this.tracing&&this.traceNode){let n=this.traceNode.scrollTop;this.lastScrollTop!==null&&(this.trace.push(Math.round(n-this.lastScrollTop)),this.trace.length>240&&this.trace.shift()),this.lastScrollTop=n}this.lastReportAt||(this.lastReportAt=t);let e=t-this.lastReportAt;if(e<500)return;let i=n=>Math.round(n*1e3/e);this.set("paint",`${i(this.paintCount)}/s`),this.set("track",`${i(this.trackCount)}/s`),this.set("model",this.inference.values.length?`${this.inference.mean.toFixed(0)} / ${this.inference.max.toFixed(0)}ms`:"\u2014"),this.set("frame",`${this.frameGaps.percentile(.5).toFixed(0)} / ${this.frameGaps.percentile(.95).toFixed(0)}ms`),this.set("worst",`${this.frameGaps.max.toFixed(0)}ms`);let s=this.paintCount?this.blockedCount/this.paintCount*100:0;this.set("blocked",`${s.toFixed(0)}%`,s>20),this.set("scroll",this.scrollCount?`${i(this.scrollCount)}/s ${this.scrollSteps.mean.toFixed(1)}px`:"\u2014");let o=this.commitReads?this.staleReads/this.commitReads*100:0;if(this.set("commit",this.commitReads?`${this.commitLag.mean.toFixed(1)}px lag, ${o.toFixed(0)}% stale`:"\u2014",this.commitLag.mean>1),this.set("target",this.targetLabel||"\u2014",!!this.targetSmooth),this.set("gesture",this.gestureLabel||"\u2014"),this.trace.length){let n=this.trace.findIndex(a=>a!==0),h=n===-1?this.trace:this.trace.slice(n),c=this.el.querySelector('[data-k="trace"]');c&&(c.textContent=h.slice(-44).join(" "))}this.paintCount=0,this.trackCount=0,this.scrollCount=0,this.blockedCount=0,this.staleReads=0,this.commitReads=0,this.lastReportAt=t}set(t,e,i=!1){let s=this.el.querySelector(`[data-k="${t}"]`);s&&(s.textContent=e,s.classList.toggle("is-warn",i))}destroy(){this.stop(),this.el.remove()}};var B=null;function Jt(r){return B||(B=import(r).catch(e=>{throw B=null,e})),B}async function $t({cdn:r,numHands:t,delegate:e}){let i=await Jt(r.vision),s=await i.FilesetResolver.forVisionTasks(r.wasm),o=n=>i.HandLandmarker.createFromOptions(s,{baseOptions:{modelAssetPath:r.model,delegate:n},runningMode:"VIDEO",numHands:t,minHandDetectionConfidence:.5,minHandPresenceConfidence:.5,minTrackingConfidence:.5});try{return await o(e)}catch(n){if(e==="CPU")throw n;return o("CPU")}}async function It(r){try{return await $t(r)}catch(t){throw console.error("[hand-cursor] could not load the MediaPipe model",t),Object.assign(t instanceof Error?t:new Error(String(t)),{code:"model"})}}async function Ct({width:r,height:t,frameRate:e}){if(!window.isSecureContext)throw Object.assign(new Error("insecure context"),{code:"insecure"});if(!navigator.mediaDevices?.getUserMedia)throw Object.assign(new Error("getUserMedia unavailable"),{code:"missing"});let i;try{i=await navigator.mediaDevices.getUserMedia({audio:!1,video:{facingMode:"user",width:{ideal:r},height:{ideal:t},frameRate:{ideal:e}}})}catch(s){let o=s?.name==="NotAllowedError"||s?.name==="SecurityError"?"denied":s?.name==="NotFoundError"||s?.name==="OverconstrainedError"?"missing":"failed";throw Object.assign(s instanceof Error?s:new Error(String(s)),{code:o})}return i}function W(r){if(r)for(let t of r.getTracks())t.stop()}var _t=400,F=class{constructor(t={}){this.options=T(S,t),this.host=document.createElement("div"),this.host.setAttribute("data-hand-cursor",""),this.shadow=this.host.attachShadow({mode:"open"});let e=document.createElement("style");e.textContent=rt,this.shadow.appendChild(e),this.panel=new M(this.options,{onToggleCamera:()=>this.running?this.stop():this.start(),onStop:()=>this.stop(),onToggleSize:()=>this.setMinimized(!this.panel.mini)}),this.panel.mount(this.shadow),this.debug=this.options.debug?new K:null,this.driver=new j(this.options,{ui:te(this.shadow),debug:this.debug,onEvent:(i,s)=>this.emit(i,s)}),this.driver.mount(this.panel.root),this.debug?.mount(this.panel.root),this.running=!1,this.starting=!1,this.destroyed=!1,this.stream=null,this.landmarker=null,this.frame=null,this.lastVideoTime=-1,this.lastHandAt=0,this.lastLandmarks=null,this.lastInferenceAt=0,this.applyStyleVariables(),this.onKeyDown=this.onKeyDown.bind(this),this.tick=this.tick.bind(this)}get position(){return this.driver.position}get pinching(){return this.driver.pinching}grabbableFrom(t){return t?R(t,this.options.grab):null}applyStyleVariables(){let{margin:t,zIndex:e,grayscale:i}=this.options;this.panel.root.style.setProperty("--hc-margin",`${t}px`),this.panel.root.style.setProperty("--hc-z",String(e)),this.panel.root.style.setProperty("--hc-video-filter",i?"grayscale(1)":"none")}mount(t=document.body){return this.mounted?this:(t.appendChild(this.host),this.mounted=!0,this.options.font&&this.injectFont(),document.addEventListener("keydown",this.onKeyDown,!0),this.options.autoStart&&this.start(),this)}injectFont(){if(document.fonts&&typeof document.fonts.check=="function"&&document.fonts.check("500 12px Inter")||document.querySelector('link[href*="family=Inter"]'))return;let e=document.createElement("link");e.rel="stylesheet",e.href=it,e.setAttribute("data-hand-cursor-font",""),document.head.appendChild(e)}onKeyDown(t){t.key==="Escape"&&this.running&&this.stop()}emit(t,e){document.dispatchEvent(new CustomEvent(`handcursor:${t}`,{detail:{...e,instance:this}}))}async start(){if(!(this.running||this.starting||this.destroyed)){this.starting=!0,this.panel.setState("loading");try{let[t,e]=await Promise.all([Ct(this.options.camera),this.landmarker??It({cdn:this.options.cdn,numHands:this.options.numHands??1,delegate:this.options.delegate})]);if(this.destroyed){W(t);return}this.stream=t,this.landmarker=e,await this.panel.attachStream(t),this.running=!0,this.starting=!1,this.lastVideoTime=-1,this.driver.reset(),this.driver.prepareHoverStyles(),this.panel.setState("live"),this.options.hideNativeCursor&&document.documentElement.style.setProperty("cursor","none","important"),this.frame=requestAnimationFrame(this.tick),this.emit("start",{})}catch(t){this.starting=!1,W(this.stream),this.stream=null;let e=this.options.strings,i=e[t?.code]||e.failed;this.panel.setState("error",i),this.emit("error",{error:t,message:i})}}}stop(){this.frame!==null&&(cancelAnimationFrame(this.frame),this.frame=null),W(this.stream),this.stream=null,this.running=!1,this.starting=!1,this.lastLandmarks=null,this.driver.release(),this.driver.reset(),this.panel.detachStream(),this.panel.setState("idle"),this.options.hideNativeCursor&&document.documentElement.style.removeProperty("cursor"),this.emit("stop",{})}setMinimized(t){this.panel.setMini(!!t),this.emit(t?"minimize":"expand",{})}destroy(){this.destroyed||(this.destroyed=!0,this.stop(),document.removeEventListener("keydown",this.onKeyDown,!0),this.landmarker?.close?.(),this.landmarker=null,this.debug?.destroy(),this.driver.destroy(),this.panel.destroy(),this.host.remove(),this.mounted=!1)}tick(t){if(!this.running)return;this.frame=requestAnimationFrame(this.tick);let e=this.panel.video;if(e.readyState<2||!e.videoWidth)return;let{maxTrackingFps:i}=this.options,s=i>0?this.lastInferenceAt+1e3/i:0;if(e.currentTime!==this.lastVideoTime&&t>=s){this.lastVideoTime=e.currentTime,this.lastInferenceAt=t;let o=this.debug?performance.now():0;try{let n=this.landmarker.detectForVideo(e,t);this.lastLandmarks=n?.landmarks?.[0]??null}catch{this.lastLandmarks=null}this.debug?.recordInference(performance.now()-o)}this.lastLandmarks?(this.lastHandAt=t,this.consumeHand(this.lastLandmarks,t,e)):t-this.lastHandAt>_t&&this.releaseHand(),this.panel.drawHand(this.lastLandmarks,this.pinching)}consumeHand(t,e,i){return this.driver.consume(t,e,i.videoWidth/i.videoHeight)}releaseHand(){this.lastLandmarks=null,this.driver.release()}};function te(r){let t=null,e=i=>{t!==i&&(t?.classList.remove("hc-hover"),i?.classList.add("hc-hover"),t=i||null)};return{contains:i=>!!(i&&r.contains(i)),hover:i=>e(i?.closest?.("button")||null),tap:i=>{let s=i?.closest?.("button");s&&!s.disabled&&s.click()}}}var St="1.0.0",w=null,Tt={};function $(r={}){w?.destroy();let t=typeof location<"u"&&location.href.includes("handcursor-debug"),e=T(Tt,t?{...r,debug:!0}:r),i=new F(e);w=i;let s=r.container||document.body;return s?i.mount(s):document.addEventListener("DOMContentLoaded",()=>{w===i&&i.mount(document.body)},{once:!0}),i}function kt(){return w}function Yt(){w?.destroy(),w=null}var ee=new Set(["autoStart","minimized","grayscale","font","hideNativeCursor"]),ie=new Set(["margin","zIndex","numHands"]);function se(r){if(!r)return{};let t={};for(let[e,i]of Object.entries(r.dataset))if(e!=="manual")if(e==="model"||e==="vision"||e==="wasm")t.cdn={...t.cdn,[e]:i};else if(ee.has(e))t[e]=i!=="false";else if(ie.has(e)){let s=Number(i);Number.isFinite(s)&&(t[e]=s)}else t[e]=i;return t}function re(r){if(Tt=se(r),r?.dataset.manual!==void 0)return;let t=()=>{w||$()};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",t,{once:!0}):t()}typeof document<"u"&&re(document.currentScript);var oe={init:$,instance:kt,destroy:Yt,DEFAULTS:S,VERSION:St};return Lt(ne);})();
