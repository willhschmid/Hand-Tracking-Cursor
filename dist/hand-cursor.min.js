/*! hand-tracking-cursor v1.0.0 | MIT | https://github.com/willhschmid/Hand-Tracking-Cursor */
var HandCursor=(()=>{var j=Object.defineProperty;var xt=Object.getOwnPropertyDescriptor;var bt=Object.getOwnPropertyNames;var yt=Object.prototype.hasOwnProperty;var At=(s,t)=>{for(var e in t)j(s,e,{get:t[e],enumerable:!0})},Ct=(s,t,e,i)=>{if(t&&typeof t=="object"||typeof t=="function")for(let n of bt(t))!yt.call(s,n)&&n!==e&&j(s,n,{get:()=>t[n],enumerable:!(i=xt(t,n))||i.enumerable});return s};var Et=s=>Ct(j({},"__esModule",{value:!0}),s);var Bt={};At(Bt,{DEFAULTS:()=>A,HandCursorController:()=>w,VERSION:()=>ft,default:()=>Wt,destroy:()=>vt,init:()=>B,instance:()=>gt});var A={position:"bottom-left",margin:16,autoStart:!1,minimized:!1,grayscale:!0,font:!0,hideNativeCursor:!1,emulateHover:!0,zIndex:2147483e3,numHands:1,debug:!1,maxTrackingFps:0,camera:{width:640,height:480,frameRate:30},region:{x:.15,y:.15},smoothing:{minCutoff:1.4,beta:.015,dCutoff:1},pinch:{on:.22,off:.32},tap:{maxDuration:800,maxTravel:32},drag:{threshold:34,holdDelay:140,holdEscape:70,mode:"write",retargetMs:70,flingScale:1,follow:.22,resample:1.35,resampleMin:24,resampleMax:90,velocityWindow:120,friction:.967,minVelocity:24,maxVelocity:3600},rotation:{enabled:!0,maxAngle:22,gain:1.8,minSpeed:.6,smoothing:.12},cursor:{scale:1,pressScale:.85},cdn:{vision:"https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@1.0.1/vision_bundle.mjs",wasm:"https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@1.0.1/wasm",model:"https://storage.googleapis.com/mediapipe-models/hand_landmarker/hand_landmarker/float16/1/hand_landmarker.task"},delegate:"GPU",strings:{intro:"Use hand tracking to control your cursor. Video never leaves your device.",enable:"Enable Camera",starting:"Starting\u2026",retry:"Try Again",minimize:"Minimize hand tracking",expand:"Expand hand tracking",disable:"Turn camera off",insecure:"Camera access needs a secure (https) connection.",denied:"Camera permission was denied. Allow access and try again.",missing:"No camera was found on this device.",model:"The hand tracking model could not load. Check your connection.",failed:"Hand tracking could not start. Please try again."}},G=s=>s!==null&&typeof s=="object"&&!Array.isArray(s);function K(s,t){let e=Array.isArray(s)?s.slice():{...s};if(!G(t))return e;for(let[i,n]of Object.entries(t))n!==void 0&&(e[i]=G(n)&&G(s?.[i])?K(s[i],n):n);return e}var d={green:"#00CA48",darkGreen:"#008630",red:"#FF4040",purple:"#FB79FF",yellow:"#FFC44F",white:"#FFFFFF",lightGray:"#F6F6F6",border:"#EBEBEB",mediumGray:"#D9D9D9",toggleGray:"#C5C5C6",iconDark:"#1C1B1F",black:"#000000",textPrimary:"rgba(0, 0, 0, 1)",textSecondary:"rgba(0, 0, 0, 0.6)",textTertiary:"rgba(0, 0, 0, 0.4)",divider:"rgba(0, 0, 0, 0.08)"},x={control:4,button:8,card:12},f={family:"Inter, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif",label:{size:12,weight:500,leading:16},body:{size:12,weight:400,leading:16}},c={panelWidth:260,panelHeight:200,miniSize:106,pad:12,gap:12,cornerInset:4,cornerButton:32,iconSize:24,ctaHeight:32,ctaPadX:12,ctaGap:8,miniCta:32,illoWidth:66,illoHeight:98},N="https://fonts.googleapis.com/css2?family=Inter:wght@400;500&display=swap";var q=`
:host {
  all: initial;
}

.hc-root {
  position: fixed;
  inset: 0;
  z-index: var(--hc-z, 2147483000);
  pointer-events: none;
  font-family: ${f.family};
  -webkit-font-smoothing: antialiased;
  color: ${d.textPrimary};
}

.hc-root * { box-sizing: border-box; }

/* ---------------------------------------------------------------- panel -- */

.hc-panel {
  position: absolute;
  pointer-events: auto;
  width: ${c.panelWidth}px;
  height: ${c.panelHeight}px;
  padding: ${c.pad}px;
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: space-between;
  background: ${d.lightGray};
  border-radius: ${x.card}px;
  overflow: hidden;
  box-shadow:
    0 0 0 1px ${d.divider},
    0 1px 2px rgba(0, 0, 0, 0.04),
    0 8px 24px rgba(0, 0, 0, 0.08);
  transition:
    width 280ms cubic-bezier(0.2, 0.8, 0.2, 1),
    height 280ms cubic-bezier(0.2, 0.8, 0.2, 1),
    background-color 200ms linear,
    opacity 200ms linear;
}

.hc-root[data-position$="-left"]  .hc-panel { left: var(--hc-margin); }
.hc-root[data-position$="-right"] .hc-panel { right: var(--hc-margin); }
.hc-root[data-position^="bottom"] .hc-panel { bottom: var(--hc-margin); }
.hc-root[data-position^="top"]    .hc-panel { top: var(--hc-margin); }

.hc-root[data-mini="true"] .hc-panel {
  width: ${c.miniSize}px;
  height: ${c.miniSize}px;
  padding: 0;
}

.hc-root[data-state="live"] .hc-panel {
  padding: 0;
  background: ${d.lightGray};
}

/* ---------------------------------------------------------------- stage -- */

.hc-stage {
  position: relative;
  /* Fixed at the illustration's size; the card distributes what is left over.
     A 98px illustration, two lines of copy and a 32px button do not leave room
     for 12px gaps inside 200px, so the gaps are what give. */
  flex: 0 0 auto;
  width: ${c.illoWidth}px;
  height: ${c.illoHeight}px;
  display: flex;
  align-items: center;
  justify-content: center;
}

.hc-root[data-state="live"] .hc-stage,
.hc-root[data-mini="true"] .hc-stage {
  position: absolute;
  inset: 0;
  width: auto;
  height: auto;
}

.hc-video,
.hc-overlay {
  position: absolute;
  inset: 0;
  width: 100%;
  height: 100%;
  display: none;
}

.hc-video {
  object-fit: cover;
  transform: scaleX(-1);
  filter: var(--hc-video-filter, none);
  /* Knocked right back so the dark icons and the skeleton stay legible over
     the #F6F6F6 card behind it. */
  opacity: var(--hc-video-opacity, 0.15);
}

.hc-root[data-state="live"] .hc-video,
.hc-root[data-state="live"] .hc-overlay { display: block; }

.hc-illo {
  display: flex;
  align-items: center;
  justify-content: center;
  width: 100%;
  height: 100%;
}

.hc-root[data-state="live"] .hc-illo { display: none; }
.hc-root[data-mini="true"] .hc-illo { padding: 10px; }

.hc-illo-img {
  display: block;
  width: 100%;
  height: 100%;
  object-fit: contain;
  user-select: none;
  -webkit-user-drag: none;
}

/* ----------------------------------------------------------------- copy -- */

.hc-copy {
  flex: none;
  margin: 0;
  max-width: 100%;
  text-align: center;
  font-size: ${f.body.size}px;
  font-weight: ${f.body.weight};
  line-height: ${f.body.leading}px;
  letter-spacing: -0.01em;
  color: ${d.textSecondary};
}

.hc-root[data-state="error"] .hc-copy { color: ${d.red}; }

.hc-root[data-state="live"] .hc-copy,
.hc-root[data-mini="true"] .hc-copy { display: none; }

/* ------------------------------------------------------------------ cta -- */

.hc-cta {
  flex: none;
  appearance: none;
  border: 0;
  margin: 0;
  display: inline-flex;
  align-items: center;
  justify-content: center;
  gap: ${c.ctaGap}px;
  height: ${c.ctaHeight}px;
  padding: 0 ${c.ctaPadX}px;
  border-radius: ${x.button}px;
  background: ${d.green};
  color: ${d.white};
  font-family: inherit;
  font-size: ${f.label.size}px;
  font-weight: ${f.label.weight};
  line-height: ${f.label.leading}px;
  letter-spacing: -0.01em;
  cursor: pointer;
  transition: background-color 150ms linear, transform 120ms ease-out;
}

.hc-cta:hover,
.hc-cta.hc-hover { background: ${d.darkGreen}; }
.hc-cta:active { transform: scale(0.97); }
.hc-cta[disabled] { opacity: 0.7; cursor: default; }

.hc-cta svg { width: ${c.iconSize}px; height: ${c.iconSize}px; flex: none; }

.hc-root[data-state="live"] .hc-cta,
.hc-root[data-mini="true"] .hc-cta { display: none; }

/* -------------------------------------------------------- corner button -- */

.hc-corner,
.hc-mini-cta {
  position: absolute;
  appearance: none;
  border: 0;
  padding: 0;
  display: none;
  align-items: center;
  justify-content: center;
  cursor: pointer;
  background: transparent;
  color: ${d.iconDark};
}

.hc-corner {
  width: ${c.cornerButton}px;
  height: ${c.cornerButton}px;
  border-radius: ${x.control}px;
  transition: background-color 150ms linear;
}

.hc-corner svg { width: ${c.iconSize}px; height: ${c.iconSize}px; }
.hc-corner:hover,
.hc-corner.hc-hover { background: ${d.divider}; }

.hc-corner--tr { top: ${c.cornerInset}px; right: ${c.cornerInset}px; display: inline-flex; }
.hc-corner--tl { top: ${c.cornerInset}px; left: ${c.cornerInset}px; }

.hc-root[data-state="live"] .hc-corner--tl { display: inline-flex; }

/* ------------------------------------------------------- minimized cta -- */

.hc-mini-cta {
  left: ${c.cornerInset}px;
  bottom: ${c.cornerInset}px;
  width: ${c.miniCta}px;
  height: ${c.miniCta}px;
  border-radius: ${x.button}px;
  background: ${d.green};
  color: ${d.white};
  transition: background-color 150ms linear, transform 120ms ease-out;
}

.hc-mini-cta svg { width: ${c.iconSize}px; height: ${c.iconSize}px; }
.hc-mini-cta:hover,
.hc-mini-cta.hc-hover { background: ${d.darkGreen}; }
.hc-mini-cta:active { transform: scale(0.94); }

.hc-root[data-mini="true"] .hc-mini-cta { display: inline-flex; }

/* Once the camera is on, the only control in the minimized card is the
   camera-off icon in the top left. The green button is the pre-enabled
   affordance and has nothing left to offer here. */
.hc-root[data-mini="true"][data-state="live"] .hc-mini-cta { display: none; }

.hc-root[data-mini="true"][data-state="loading"] .hc-mini-cta { opacity: 0.7; }

/* -------------------------------------------------------------- spinner -- */

.hc-spinner {
  width: ${c.iconSize}px;
  height: ${c.iconSize}px;
  flex: none;
  border-radius: 50%;
  border: 2px solid rgba(255, 255, 255, 0.35);
  border-top-color: ${d.white};
  animation: hc-spin 700ms linear infinite;
}

@keyframes hc-spin { to { transform: rotate(360deg); } }

/* --------------------------------------------------------------- cursor -- */

.hc-cursor {
  position: fixed;
  top: 0;
  left: 0;
  width: var(--hc-cursor-w, 20px);
  aspect-ratio: 1;
  /* The arrow tip sits at the element's origin, so rotation and the tapped
     scale both pivot on the exact point being addressed. */
  transform-origin: 0 0;
  color: ${d.black};
  opacity: 0;
  will-change: transform, opacity;
  transition: opacity 160ms linear;
}

/* display:block matters here: an inline svg sits on a text baseline and picks
   up descender space, which made the element taller than its aspect-ratio. */
.hc-cursor svg { display: block; width: 100%; height: 100%; overflow: visible; }

.hc-cursor[data-visible="true"] { opacity: 1; }

/* ---------------------------------------------------------------- debug -- */

.hc-debug {
  position: fixed;
  top: 8px;
  left: 8px;
  pointer-events: none;
  min-width: 168px;
  padding: 8px 10px;
  border-radius: ${x.button}px;
  background: rgba(0, 0, 0, 0.82);
  color: ${d.white};
  font-family: ui-monospace, SFMono-Regular, Menlo, monospace;
  font-size: 11px;
  line-height: 15px;
  font-variant-numeric: tabular-nums;
}

.hc-debug-row { display: flex; justify-content: space-between; gap: 12px; }
.hc-debug-row span { opacity: 0.55; }
.hc-debug-row b { font-weight: 500; }
.hc-debug-row b.is-warn { color: ${d.yellow}; }

.hc-debug-trace {
  margin-top: 6px;
  padding-top: 6px;
  border-top: 1px solid rgba(255, 255, 255, 0.18);
  max-width: 240px;
}

.hc-debug-trace span { display: block; opacity: 0.55; }

.hc-debug-trace code {
  display: block;
  font: inherit;
  word-spacing: 2px;
  line-height: 14px;
  overflow-wrap: anywhere;
}

/* ------------------------------------------------------------------ a11y -- */

.hc-sr {
  position: absolute;
  width: 1px;
  height: 1px;
  margin: -1px;
  padding: 0;
  overflow: hidden;
  clip: rect(0 0 0 0);
  white-space: nowrap;
  border: 0;
}

.hc-root :focus-visible {
  outline: 2px solid ${d.green};
  outline-offset: 2px;
}

@media (prefers-reduced-motion: reduce) {
  .hc-panel,
  .hc-cta,
  .hc-mini-cta,
  .hc-corner { transition-duration: 1ms; }
  .hc-spinner { animation-duration: 2s; }
}
`;var I=s=>`<svg viewBox="0 0 24 24" fill="none" aria-hidden="true" focusable="false"><path d="${s}" fill="currentColor"/></svg>`,v={videocam:I("M4 20C3.45 20 2.97917 19.8042 2.5875 19.4125C2.19583 19.0208 2 18.55 2 18V6C2 5.45 2.19583 4.97917 2.5875 4.5875C2.97917 4.19583 3.45 4 4 4H16C16.55 4 17.0208 4.19583 17.4125 4.5875C17.8042 4.97917 18 5.45 18 6V10.5L21.15 7.35C21.3167 7.18333 21.5 7.14167 21.7 7.225C21.9 7.30833 22 7.46667 22 7.7V16.3C22 16.5333 21.9 16.6917 21.7 16.775C21.5 16.8583 21.3167 16.8167 21.15 16.65L18 13.5V18C18 18.55 17.8042 19.0208 17.4125 19.4125C17.0208 19.8042 16.55 20 16 20H4ZM4 18H16V6H4V18Z"),videocamOff:I("M18.0002 10.5L21.1502 7.35001C21.3169 7.18334 21.5002 7.14167 21.7002 7.22501C21.9002 7.30834 22.0002 7.46667 22.0002 7.70001V16.3C22.0002 16.5333 21.9002 16.6917 21.7002 16.775C21.5002 16.8583 21.3169 16.8167 21.1502 16.65L18.0002 13.5C18.0002 13.7833 17.9044 14.0208 17.7127 14.2125C17.521 14.4042 17.2835 14.5 17.0002 14.5C16.7169 14.5 16.4794 14.4042 16.2877 14.2125C16.096 14.0208 16.0002 13.7833 16.0002 13.5V6.00001H9.0002C8.66686 6.00001 8.41686 5.89584 8.2502 5.68751C8.08353 5.47917 8.0002 5.25001 8.0002 5.00001C8.0002 4.75001 8.08353 4.52084 8.2502 4.31251C8.41686 4.10417 8.66686 4.00001 9.0002 4.00001H16.0002C16.5502 4.00001 17.021 4.19584 17.4127 4.58751C17.8044 4.97917 18.0002 5.45001 18.0002 6.00001V10.5ZM19.8502 22.65L1.3502 4.15001C1.16686 3.96667 1.0752 3.73334 1.0752 3.45001C1.0752 3.16667 1.16686 2.93334 1.3502 2.75001C1.53353 2.56667 1.76686 2.47501 2.0502 2.47501C2.33353 2.47501 2.56686 2.56667 2.7502 2.75001L21.2502 21.25C21.4335 21.4333 21.5252 21.6667 21.5252 21.95C21.5252 22.2333 21.4335 22.4667 21.2502 22.65C21.0669 22.8333 20.8335 22.925 20.5502 22.925C20.2669 22.925 20.0335 22.8333 19.8502 22.65ZM4.0002 4.00001L6.0002 6.00001H4.0002V18H16.0002V16L18.0002 18C18.0002 18.55 17.8044 19.0208 17.4127 19.4125C17.021 19.8042 16.5502 20 16.0002 20H4.0002C3.4502 20 2.97936 19.8042 2.5877 19.4125C2.19603 19.0208 2.0002 18.55 2.0002 18V6.00001C2.0002 5.45001 2.19603 4.97917 2.5877 4.58751C2.97936 4.19584 3.4502 4.00001 4.0002 4.00001Z"),collapse:I("M9.92658 15H6.91499C6.63057 15 6.39215 14.9042 6.19974 14.7125C6.00734 14.5208 5.91113 14.2833 5.91113 14C5.91113 13.7167 6.00734 13.4792 6.19974 13.2875C6.39215 13.0958 6.63057 13 6.91499 13H10.9304C11.2149 13 11.4533 13.0958 11.6457 13.2875C11.8381 13.4792 11.9343 13.7167 11.9343 14V18C11.9343 18.2833 11.8381 18.5208 11.6457 18.7125C11.4533 18.9042 11.2149 19 10.9304 19C10.646 19 10.4076 18.9042 10.2152 18.7125C10.0228 18.5208 9.92658 18.2833 9.92658 18V15ZM15.9497 9H18.9613C19.2458 9 19.4842 9.09583 19.6766 9.2875C19.869 9.47917 19.9652 9.71667 19.9652 10C19.9652 10.2833 19.869 10.5208 19.6766 10.7125C19.4842 10.9042 19.2458 11 18.9613 11H14.9459C14.6615 11 14.423 10.9042 14.2306 10.7125C14.0382 10.5208 13.942 10.2833 13.942 10V6C13.942 5.71667 14.0382 5.47917 14.2306 5.2875C14.423 5.09583 14.6615 5 14.9459 5C15.2303 5 15.4687 5.09583 15.6611 5.2875C15.8535 5.47917 15.9497 5.71667 15.9497 6V9Z"),expand:I("M7 17H10C10.2833 17 10.5208 17.0958 10.7125 17.2875C10.9042 17.4792 11 17.7167 11 18C11 18.2833 10.9042 18.5208 10.7125 18.7125C10.5208 18.9042 10.2833 19 10 19H6C5.71667 19 5.47917 18.9042 5.2875 18.7125C5.09583 18.5208 5 18.2833 5 18V14C5 13.7167 5.09583 13.4792 5.2875 13.2875C5.47917 13.0958 5.71667 13 6 13C6.28333 13 6.52083 13.0958 6.7125 13.2875C6.90417 13.4792 7 13.7167 7 14V17ZM17 7H14C13.7167 7 13.4792 6.90417 13.2875 6.7125C13.0958 6.52083 13 6.28333 13 6C13 5.71667 13.0958 5.47917 13.2875 5.2875C13.4792 5.09583 13.7167 5 14 5H18C18.2833 5 18.5208 5.09583 18.7125 5.2875C18.9042 5.47917 19 5.71667 19 6V10C19 10.2833 18.9042 10.5208 18.7125 10.7125C18.5208 10.9042 18.2833 11 18 11C17.7167 11 17.4792 10.9042 17.2875 10.7125C17.0958 10.5208 17 10.2833 17 10V7Z")},J='<svg viewBox="9.6 9.6 14 14" fill="none" aria-hidden="true" focusable="false"><path d="M9.40234 11.3525C8.91256 10.1281 10.1281 8.91256 11.3525 9.40234L22.6514 13.9219C23.9484 14.441 23.8937 16.2954 22.5684 16.7373L18.4326 18.1162C18.2833 18.166 18.166 18.2833 18.1162 18.4326L16.7373 22.5684C16.2954 23.8937 14.441 23.9484 13.9219 22.6514L9.40234 11.3525Z" fill="#111111" stroke="white"/></svg>';var $=[[0,1],[1,2],[2,3],[3,4],[0,5],[5,6],[6,7],[7,8],[9,10],[10,11],[11,12],[13,14],[14,15],[15,16],[0,17],[17,18],[18,19],[19,20],[5,9],[9,13],[13,17]];function _(s,t,e){let i=(s.x-t.x)*e,n=s.y-t.y;return Math.hypot(i,n)}function wt(s,t=1){return Math.max(_(s[0],s[9],t),1e-4)}function tt(s,t=1){return _(s[4],s[8],t)/wt(s,t)}function et(s){let t=s[4],e=s[8];return{x:(t.x+e.x)/2,y:(t.y+e.y)/2}}var V=3.2,It=1.6;function nt(s,t,{pinching:e=!1}={}){let i=e?d.green:d.purple;s.lineCap="round",s.lineJoin="round",s.strokeStyle=i,s.lineWidth=It,s.beginPath();for(let[o,r]of $)s.moveTo(t[o].x,t[o].y),s.lineTo(t[r].x,t[r].y);s.stroke(),s.strokeStyle=d.green,s.lineWidth=1.4,s.setLineDash(e?[]:[3,3]),s.beginPath(),s.moveTo(t[8].x,t[8].y),s.lineTo(t[4].x,t[4].y),s.stroke(),s.setLineDash([]),s.fillStyle=i;let n=V/2;for(let o of t)s.fillRect(o.x-n,o.y-n,V,V)}var ot="data:image/png;base64,";var S=class{constructor(t,e){this.options=t,this.handlers=e,this.state="idle",this.mini=!!t.minimized,this.root=document.createElement("div"),this.root.className="hc-root",this.root.dataset.position=t.position,this.root.dataset.state=this.state,this.root.dataset.mini=String(this.mini),this.root.innerHTML=`
      <div class="hc-panel" role="region" aria-label="Hand tracking trackpad">
        <div class="hc-stage">
          <video class="hc-video" playsinline muted autoplay></video>
          <canvas class="hc-overlay" aria-hidden="true"></canvas>
          <div class="hc-illo"><img class="hc-illo-img" src="${ot}" alt="" width="${c.illoWidth}" height="${c.illoHeight}" draggable="false" /></div>
        </div>
        <p class="hc-copy"></p>
        <button class="hc-cta" type="button"></button>
        <button class="hc-corner hc-corner--tl" type="button"></button>
        <button class="hc-corner hc-corner--tr" type="button"></button>
        <button class="hc-mini-cta" type="button"></button>
      </div>
      <p class="hc-sr" role="status" aria-live="polite"></p>
    `;let i=n=>this.root.querySelector(n);this.panel=i(".hc-panel"),this.video=i(".hc-video"),this.canvas=i(".hc-overlay"),this.copy=i(".hc-copy"),this.cta=i(".hc-cta"),this.cornerLeft=i(".hc-corner--tl"),this.cornerRight=i(".hc-corner--tr"),this.miniCta=i(".hc-mini-cta"),this.status=i(".hc-sr"),this.ctx=this.canvas.getContext("2d"),this.canvasSize={width:0,height:0,dpr:0},this.cornerLeft.innerHTML=v.videocamOff,this.cornerLeft.title=t.strings.disable,this.cornerLeft.setAttribute("aria-label",t.strings.disable),this.cta.addEventListener("click",n=>e.onToggleCamera(n)),this.miniCta.addEventListener("click",n=>e.onToggleCamera(n)),this.cornerLeft.addEventListener("click",()=>e.onStop()),this.cornerRight.addEventListener("click",()=>e.onToggleSize()),this.render()}mount(t){t.appendChild(this.root)}setState(t,e){this.state=t,this.root.dataset.state=t,this.message=e,this.render()}setMini(t){this.mini=t,this.root.dataset.mini=String(t),this.render()}render(){let t=this.options.strings,e=this.state==="loading",i=this.state==="live";this.copy.textContent=this.state==="error"?this.message||t.failed:t.intro;let n=e?t.starting:this.state==="error"?t.retry:t.enable;this.cta.innerHTML=(e?'<span class="hc-spinner"></span>':v.videocam)+`<span>${n}</span>`,this.cta.disabled=e,this.miniCta.innerHTML=i?v.videocamOff:v.videocam;let o=i?t.disable:t.enable;this.miniCta.title=o,this.miniCta.setAttribute("aria-label",o),this.miniCta.disabled=e;let r=this.mini?t.expand:t.minimize;this.cornerRight.innerHTML=this.mini?v.expand:v.collapse,this.cornerRight.title=r,this.cornerRight.setAttribute("aria-label",r),this.status.textContent=i?"Hand tracking is on.":this.state==="error"?this.message||t.failed:""}attachStream(t){return this.video.srcObject=t,this.video.play().catch(()=>{})}detachStream(){this.video.srcObject=null,this.clearOverlay()}syncCanvas(){let t=Math.min(window.devicePixelRatio||1,2),e=this.canvas.clientWidth,i=this.canvas.clientHeight;if(!e||!i)return null;let n=this.canvasSize;return(n.width!==e||n.height!==i||n.dpr!==t)&&(this.canvas.width=Math.round(e*t),this.canvas.height=Math.round(i*t),this.canvasSize={width:e,height:i,dpr:t}),{width:e,height:i,dpr:t}}clearOverlay(){this.canvas.width&&(this.ctx.setTransform(1,0,0,1,0,0),this.ctx.clearRect(0,0,this.canvas.width,this.canvas.height))}drawHand(t,e){let i=this.syncCanvas();if(!i)return;let{videoWidth:n,videoHeight:o}=this.video;if(this.ctx.setTransform(i.dpr,0,0,i.dpr,0,0),this.ctx.clearRect(0,0,i.width,i.height),!t||!n||!o)return;let r=Math.max(i.width/n,i.height/o),h=n*r,l=o*r,a=(i.width-h)/2,u=(i.height-l)/2,p=t.map(m=>({x:a+(1-m.x)*h,y:u+m.y*l}));nt(this.ctx,p,{pinching:e})}destroy(){this.detachStream(),this.root.remove()}};var St=(s,t,e)=>s<t?t:s>e?e:s,Y=class{constructor(t){this.options=t,this.el=document.createElement("div"),this.el.className="hc-cursor",this.el.setAttribute("aria-hidden","true"),this.el.innerHTML=J,this.x=0,this.y=0,this.angle=0,this.scale=t.cursor.scale,this.visible=!1,this.pressed=!1,this.lastMoveAt=0,this.reducedMotion=typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches}mount(t){t.appendChild(this.el)}setVisible(t){this.visible!==t&&(this.visible=t,this.el.dataset.visible=String(t))}setPressed(t){this.pressed=t}update(t,e,i,n,o){this.x=t,this.y=e;let r=this.options.rotation;if(r.enabled&&!this.reducedMotion){let l=Math.abs(i)<r.minSpeed?0:St(i*r.gain,-r.maxAngle,r.maxAngle);this.angle+=(l-this.angle)*r.smoothing}else this.angle=0;let h=this.pressed?this.options.cursor.pressScale:this.options.cursor.scale;this.scale+=(h-this.scale)*.35,this.el.style.transform=`translate3d(${t.toFixed(2)}px, ${e.toFixed(2)}px, 0) rotate(${this.angle.toFixed(2)}deg) scale(${this.scale.toFixed(3)})`}reset(){this.angle=0,this.scale=this.options.cursor.scale,this.pressed=!1}destroy(){this.el.remove()}};var rt=/(auto|scroll|overlay)/;function ct(s){let t=s;for(;t&&t!==document.documentElement&&t!==document.body;){if(t.nodeType===1){let i=getComputedStyle(t),n=rt.test(i.overflowY)&&t.scrollHeight-t.clientHeight>1,o=rt.test(i.overflowX)&&t.scrollWidth-t.clientWidth>1;if(n||o)return{node:t,canX:o,canY:n}}t=t.parentElement||t.getRootNode()?.host||null}let e=document.scrollingElement||document.documentElement;return{node:e,canX:e.scrollWidth-e.clientWidth>1,canY:e.scrollHeight-e.clientHeight>1}}function Yt(s){let t=s===document.scrollingElement?document.documentElement:s,e=t.style.scrollBehavior;return t.style.setProperty("scroll-behavior","auto","important"),()=>{e?t.style.setProperty("scroll-behavior",e,""):t.style.removeProperty("scroll-behavior")}}function Ft(s){let t=s===document.scrollingElement?document.documentElement:s;return getComputedStyle(t).scrollBehavior==="smooth"}function F(s){let t=window.devicePixelRatio||1;return Math.round(s*t)/t}function at(s){return{x:Math.max(0,s.scrollWidth-s.clientWidth),y:Math.max(0,s.scrollHeight-s.clientHeight)}}function lt(s){return-1/(60*Math.log(s))}function ht(s,t,e){return s*lt(t)*e}var Mt=.05,kt=.15,M=class{constructor(t,e){this.options=t,this.debug=e,this.target=null,this.askedX=0,this.askedY=0,this.appliedX=0,this.appliedY=0,this.path=[],this.interval=0,this.head=0,this.positionX=0,this.positionY=0,this.live=!1,this.velocityX=0,this.velocityY=0,this.flinging=!1,this.frame=null,this.lastFrameAt=0,this.restoreBehavior=null,this.lastRetargetAt=0,this.tick=this.tick.bind(this)}resetPath(){this.askedX=0,this.askedY=0,this.appliedX=0,this.appliedY=0,this.path=[],this.interval=0,this.head=0,this.live=!1}setTarget(t){t!==this.target&&(this.resetPath(),this.releaseBehavior()),this.target=t,t&&(this.claimPosition(),this.debug?.recordTarget(t.node,Ft(t.node)),this.debug?.beginTrace(t.node),this.lastRetargetAt=0,(this.options.drag.mode==="write"||this.options.drag.mode==="hybrid")&&(this.restoreBehavior||(this.restoreBehavior=Yt(t.node))))}releaseBehavior(){this.restoreBehavior?.(),this.restoreBehavior=null}claimPosition(){this.target&&(this.positionX=this.target.node.scrollLeft,this.positionY=this.target.node.scrollTop)}applyScroll(t,e){let{node:i,canX:n,canY:o}=this.target,r=at(i);o&&(this.positionY=Math.max(0,Math.min(r.y,this.positionY-e))),n&&(this.positionX=Math.max(0,Math.min(r.x,this.positionX-t)));let h=F(this.positionY),l=F(this.positionX);try{i.scrollTo({top:h,left:l,behavior:"instant"})}catch{i.scrollTop=h,i.scrollLeft=l}this.debug&&this.debug.recordCommit(h,o?i.scrollTop:i.scrollLeft)}push(t,e,i){if(!this.target)return;this.flinging=!1,this.live=!0;let n=this.path.at(-1);if(n){let o=i-n.t;o>=8&&o<=250&&(this.interval=this.interval?this.interval*.7+o*.3:o)}if(this.path.length===0){let o=this.options.drag.resampleMax;this.path.push({t:i-o,x:this.askedX,y:this.askedY})}for(this.askedX+=t,this.askedY+=e,this.path.push({t:i,x:this.askedX,y:this.askedY});this.path.length>2&&i-this.path[0].t>1e3;)this.path.shift();this.options.drag.mode==="native"?this.retarget():this.start()}delay(){let{resample:t,resampleMin:e,resampleMax:i}=this.options.drag;return this.interval?Math.max(e,Math.min(i,this.interval*t)):i}advanceHead(t,e){let i=t-this.delay();if(!this.head)return this.head=i,this.head;this.head+=e;let n=e*Mt,o=(i-this.head)*kt;return this.head+=Math.max(-n,Math.min(n,o)),this.head}positionAt(t){let e=this.path;if(e.length===0)return{x:this.appliedX,y:this.appliedY};let i=e[0];if(t<=i.t)return{x:i.x,y:i.y};let n=e.at(-1);if(t>=n.t){let u=e.at(-2),p=u?n.t-u.t:0;if(!this.live||!u||p<=0)return{x:n.x,y:n.y};let m=Math.min(t-n.t,this.options.drag.resampleMax)/p;return{x:n.x+(n.x-u.x)*m,y:n.y+(n.y-u.y)*m}}let o=e.length-1;for(;o>0&&e[o-1].t>t;)o-=1;let r=e[o-1],h=e[o],l=h.t-r.t,a=l>0?(t-r.t)/l:1;return{x:r.x+(h.x-r.x)*a,y:r.y+(h.y-r.y)*a}}retarget(t=!1,e="smooth"){let i=performance.now(),{retargetMs:n}=this.options.drag;if(!t&&i-this.lastRetargetAt<n)return!0;this.lastRetargetAt=i;let{node:o,canX:r,canY:h}=this.target,l=at(o);h&&(this.positionY=Math.max(0,Math.min(l.y,this.positionY-this.remainingY))),r&&(this.positionX=Math.max(0,Math.min(l.x,this.positionX-this.remainingX)));let a=F(this.positionY),u=F(this.positionX);try{o.scrollTo({top:a,left:u,behavior:e})}catch{return!1}return this.appliedX=this.askedX,this.appliedY=this.askedY,this.debug?.recordScroll(0),!0}fling(t,e){let{minVelocity:i,maxVelocity:n,mode:o,friction:r,flingScale:h}=this.options.drag,l=U=>Math.max(-n,Math.min(n,U)),a=l(t),u=l(e),p=Math.hypot(a,u)<i;if(this.live=!1,(o==="native"||o==="hybrid")&&(p||(this.askedX+=ht(a,r,h),this.askedY+=ht(u,r,h)),this.releaseBehavior(),this.retarget(!0,"smooth"))){this.stop();return}if(p)return;let m=lt(r);this.velocityX=a+this.remainingX/m,this.velocityY=u+this.remainingY/m,this.appliedX=this.askedX,this.appliedY=this.askedY,this.flinging=!0,this.start()}start(){this.frame===null&&(this.lastFrameAt=0,this.frame=requestAnimationFrame(this.tick))}stop(){this.frame!==null&&(cancelAnimationFrame(this.frame),this.frame=null),this.flinging=!1,this.resetPath(),this.velocityX=0,this.velocityY=0,this.releaseBehavior()}tick(t){if(this.frame=null,!this.target)return;let e=this.lastFrameAt?Math.min(t-this.lastFrameAt,50):1e3/60,i=e/1e3;this.lastFrameAt=t;let n=0,o=0;if(this.flinging){let{friction:h,minVelocity:l}=this.options.drag,a=h**(i*60);this.velocityX*=a,this.velocityY*=a,n=this.velocityX*i,o=this.velocityY*i,Math.hypot(this.velocityX,this.velocityY)<l&&(this.flinging=!1)}else if(this.options.drag.follow>=1)n=this.remainingX,o=this.remainingY,this.appliedX=this.askedX,this.appliedY=this.askedY;else{let h=this.positionAt(this.advanceHead(t,e));n=h.x-this.appliedX,o=h.y-this.appliedY,this.appliedX=h.x,this.appliedY=h.y}(n||o)&&(this.applyScroll(n,o),this.debug?.recordScroll(o||n));let r=Math.abs(this.remainingX)<.001&&Math.abs(this.remainingY)<.001;!this.flinging&&r?(this.releaseBehavior(),this.debug?.endTrace()):this.frame=requestAnimationFrame(this.tick)}get remainingX(){return this.askedX-this.appliedX}get remainingY(){return this.askedY-this.appliedY}};var zt="a[href], button, input, select, textarea, summary, [tabindex]",Ot=1e3;function Tt(s,t){let e=document.elementFromPoint(s,t);for(;e?.shadowRoot;){let i=e.shadowRoot.elementFromPoint(s,t);if(!i||i===e)break;e=i}return e}function k(s,t,e){return{bubbles:!0,cancelable:!0,composed:!0,view:window,clientX:s,clientY:t,screenX:(window.screenX||0)+s,screenY:(window.screenY||0)+t,...e}}var Rt={pointerId:1,pointerType:"touch",isPrimary:!0,width:1,height:1};function C(s,t,e,i,n){s.dispatchEvent(new PointerEvent(t,k(e,i,{...Rt,...n})))}function b(s,t,e,i,n){s.dispatchEvent(new MouseEvent(t,k(e,i,n)))}var z=class{constructor(t,{ui:e,onTap:i,hover:n,debug:o}={}){this.options=t,this.ui=e,this.onTap=i,this.hoverStyles=n,this.hovered=null,this.pressing=!1,this.dragging=!1,this.origin=null,this.last=null,this.samples=[],this.scrollTarget=null,this.scroller=new M(t,o)}resolve(t,e){let i=Tt(t,e);return{el:i,internal:!!this.ui?.contains(i)}}move(t,e){let{el:i,internal:n}=this.resolve(t,e);if(this.ui?.hover(n?i:null,t,e),n||!i){this.leaveHovered(t,e);return}i!==this.hovered&&(this.leaveHovered(t,e),this.hovered=i,this.hoverStyles?.set(i),C(i,"pointerover",t,e,{buttons:this.pressing?1:0}),b(i,"mouseover",t,e,{buttons:this.pressing?1:0}),i.dispatchEvent(new MouseEvent("mouseenter",{...k(t,e),bubbles:!1,cancelable:!1}))),C(i,"pointermove",t,e,{buttons:this.pressing?1:0}),b(i,"mousemove",t,e,{buttons:this.pressing?1:0})}leaveHovered(t,e){let i=this.hovered;i&&(this.hovered=null,this.hoverStyles?.clear(),C(i,"pointerout",t,e,{buttons:0}),b(i,"mouseout",t,e,{buttons:0}),i.dispatchEvent(new MouseEvent("mouseleave",{...k(t,e),bubbles:!1,cancelable:!1})))}press(t,e,i){this.scroller.stop();let{el:n,internal:o}=this.resolve(t,e);this.pressing=!0,this.dragging=!1,this.origin={x:t,y:e,t:i,el:n,internal:o},this.last={x:t,y:e,t:i},this.samples=[{x:t,y:e,t:i}],this.scrollTarget=o||!n?null:ct(n),this.scroller.setTarget(this.scrollTarget)}drag(t,e,i){if(!this.pressing)return;let n=t-this.last.x,o=e-this.last.y;this.last={x:t,y:e,t:i},this.samples.push({x:t,y:e,t:i});let r=this.options.drag.velocityWindow*2;for(;this.samples.length>2&&i-this.samples[0].t>r;)this.samples.shift();if(!this.dragging){let{threshold:h,holdDelay:l,holdEscape:a}=this.options.drag,u=Math.hypot(t-this.origin.x,e-this.origin.y);if(u<a&&i-this.origin.t<l||u<h)return;this.dragging=!0,this.leaveHovered(t,e)}this.scroller.push(n,o,i)}releaseVelocity(){let t=this.samples.at(-1);if(!t)return{x:0,y:0};let e=t.t-this.options.drag.velocityWindow,i=this.samples.length-1;for(;i>0&&this.samples[i-1].t>=e;)i-=1;let n=this.samples[Math.max(i-1,0)],o=(t.t-n.t)/Ot;return o<=0?{x:0,y:0}:{x:(t.x-n.x)/o,y:(t.y-n.y)/o}}release(t,e,i){if(!this.pressing)return;let n=this.origin;if(this.pressing=!1,this.origin=null,this.dragging){this.dragging=!1;let a=this.releaseVelocity();this.scroller.fling(a.x,a.y);return}let o=Math.hypot(t-n.x,e-n.y),r=i-n.t,{maxDuration:h,maxTravel:l}=this.options.tap;o>l||r>h||this.tap(n.x,n.y)}tap(t,e){let{el:i,internal:n}=this.resolve(t,e);if(!i)return;if(n){this.ui.tap(i,t,e),this.onTap?.({x:t,y:e,target:i,internal:!0});return}C(i,"pointerdown",t,e,{buttons:1,button:0}),b(i,"mousedown",t,e,{buttons:1,button:0});let o=i.closest?.(zt);if(o)try{o.focus({preventScroll:!0})}catch{}else document.activeElement&&document.activeElement!==document.body&&document.activeElement.blur?.();C(i,"pointerup",t,e,{buttons:0,button:0}),b(i,"mouseup",t,e,{buttons:0,button:0}),b(i,"click",t,e,{buttons:0,button:0,detail:1}),this.onTap?.({x:t,y:e,target:i,internal:!1})}cancel(t=0,e=0){this.scroller.stop(),this.pressing=!1,this.dragging=!1,this.origin=null,this.scrollTarget=null,this.ui?.hover(null,t,e),this.leaveHovered(t,e)}destroy(){this.cancel()}};function dt(s,t){return 1/(1+1/(2*Math.PI*s)/t)}var O=class{constructor(){this.value=null}filter(t,e){return this.value=this.value===null?t:e*t+(1-e)*this.value,this.value}reset(){this.value=null}},T=class{constructor({minCutoff:t=1,beta:e=0,dCutoff:i=1}={}){this.minCutoff=t,this.beta=e,this.dCutoff=i,this.x=new O,this.dx=new O}filter(t,e){e>0||(e=1/60);let i=this.x.value,n=i===null?0:(t-i)/e,o=this.dx.filter(n,dt(this.dCutoff,e)),r=this.minCutoff+this.beta*Math.abs(o);return this.x.filter(t,dt(r,e))}reset(){this.x.reset(),this.dx.reset()}},E=class{constructor(t){this.fx=new T(t),this.fy=new T(t)}filter(t,e,i){return{x:this.fx.filter(t,i),y:this.fy.filter(e,i)}}reset(){this.fx.reset(),this.fy.reset()}};var R="data-hc-hover",Pt=s=>s.replace(/:hover\b/g,`[${R}]`);function W(s,t){for(let e of s)if(e.selectorText)e.selectorText.includes(":hover")&&t.push(`${Pt(e.selectorText)}{${e.style.cssText}}`);else if(e.media){let i=[];W(e.cssRules,i),i.length&&t.push(`@media ${e.conditionText}{${i.join("")}}`)}else if(e.conditionText&&e.cssRules){let i=[];W(e.cssRules,i),i.length&&t.push(`@supports ${e.conditionText}{${i.join("")}}`)}}var P=class{constructor(){this.style=null,this.chain=[],this.sheetCount=-1,this.warned=!1,this.observer=null}build(){let t=[],e=0;for(let i of document.styleSheets)if(i.ownerNode!==this.style)try{W(i.cssRules,t)}catch{e+=1}e&&!this.warned&&(this.warned=!0,console.info(`[hand-cursor] ${e} cross-origin stylesheet(s) could not be read, so their :hover styles will not respond to the hand cursor.`)),this.style||(this.style=document.createElement("style"),this.style.setAttribute("data-hand-cursor-hover","")),this.style.textContent=t.join(`
`),document.head.appendChild(this.style),this.sheetCount=document.styleSheets.length,this.observer||this.watch()}watch(){let t=0;this.observer=new MutationObserver(()=>{document.styleSheets.length!==this.sheetCount&&(clearTimeout(t),t=setTimeout(()=>this.build(),250))}),this.observer.observe(document.documentElement,{childList:!0,subtree:!0})}set(t){if(this.chain[0]===t)return;let e=[];for(let i=t;i&&i.nodeType===1;i=i.parentElement)e.push(i);for(let i of this.chain)e.includes(i)||i.removeAttribute(R);for(let i of e)i.setAttribute(R,"");this.chain=e}clear(){for(let t of this.chain)t.removeAttribute(R);this.chain=[]}destroy(){this.clear(),this.observer?.disconnect(),this.observer=null,this.style?.remove(),this.style=null}};var ut=s=>s<0?0:s>1?1:s,L=class{constructor(t,{ui:e,onEvent:i,debug:n}={}){this.options=t,this.onEvent=i,this.debug=n,this.cursor=new Y(t),this.hoverStyles=t.emulateHover?new P:null,this.touch=new z(t,{ui:e,debug:n,hover:this.hoverStyles,onTap:o=>this.emit("tap",o)}),this.filter=new E(t.smoothing),this.position=null,this.velocity={x:0,y:0},this.pinching=!1,this.lastFrameAt=0}mount(t){this.cursor.mount(t)}emit(t,e){this.onEvent?.(t,e)}consume(t,e,i=1){this.debug?.recordTracked();let n=this.lastFrameAt?(e-this.lastFrameAt)/1e3:1/60;this.lastFrameAt=e;let o=et(t),r=this.options.region,h=ut((1-o.x-r.x)/(1-2*r.x)),l=ut((o.y-r.y)/(1-2*r.y)),a=this.filter.filter(h*window.innerWidth,l*window.innerHeight,n),u=this.position;this.position=a,this.velocity=u?{x:a.x-u.x,y:a.y-u.y}:{x:0,y:0},this.cursor.setVisible(!0),this.cursor.update(a.x,a.y,this.velocity.x,this.velocity.y,e);let p=tt(t,i),{on:m,off:U}=this.options.pinch,D=this.pinching?p<U:p<m;D&&!this.pinching?(this.pinching=!0,this.cursor.setPressed(!0),this.touch.press(a.x,a.y,e),this.emit("press",{x:a.x,y:a.y})):!D&&this.pinching&&(this.pinching=!1,this.cursor.setPressed(!1),this.touch.release(a.x,a.y,e),this.emit("release",{x:a.x,y:a.y})),this.pinching?this.touch.drag(a.x,a.y,e):this.touch.move(a.x,a.y);let Z={x:a.x,y:a.y,pinching:this.pinching,ratio:p};return this.emit("move",Z),Z}release(){this.position?this.touch.cancel(this.position.x,this.position.y):this.touch.cancel(),this.pinching=!1,this.position=null,this.lastFrameAt=0,this.filter.reset(),this.cursor.setPressed(!1),this.cursor.setVisible(!1)}reset(){this.filter.reset(),this.cursor.reset(),this.lastFrameAt=0}setSmoothing(t){this.options.smoothing={...this.options.smoothing,...t},this.filter=new E(this.options.smoothing),this.lastFrameAt=0}prepareHoverStyles(){this.hoverStyles?.build()}destroy(){this.touch.destroy(),this.cursor.destroy(),this.hoverStyles?.destroy()}};var y=class{constructor(){this.values=[]}push(t){this.values.push(t),this.values.length>60&&this.values.shift()}get mean(){return this.values.length?this.values.reduce((t,e)=>t+e,0)/this.values.length:0}percentile(t){if(!this.values.length)return 0;let e=[...this.values].sort((i,n)=>i-n);return e[Math.min(e.length-1,Math.floor(e.length*t))]}get max(){return this.values.length?Math.max(...this.values):0}},Lt=[["paint","repaints per second \u2014 the rate the screen can actually update"],["track","landmark frames per second"],["model","milliseconds per inference, mean / worst"],["frame","milliseconds between repaints, median / 95th"],["worst","longest gap between repaints"],["blocked","share of repaints later than 32ms"],["scroll","scroll writes per second, and mean step"],["commit","how far the browser\u2019s reported scroll offset trails what was just written, and how often it had not moved at all. Anything but 0 means this platform commits scrolls asynchronously"],["target","what is being scrolled, and whether its CSS asked for smooth"]],X=class{constructor(){this.inference=new y,this.frameGaps=new y,this.scrollSteps=new y,this.commitLag=new y,this.staleReads=0,this.commitReads=0,this.lastReported=null,this.paintCount=0,this.trackCount=0,this.scrollCount=0,this.blockedCount=0,this.lastPaintAt=0,this.lastReportAt=0,this.running=!1,this.targetLabel="",this.targetSmooth=!1,this.trace=[],this.traceNode=null,this.lastScrollTop=null,this.tracing=!1,this.el=document.createElement("div"),this.el.className="hc-debug",this.el.innerHTML=Lt.map(([t,e])=>`<div class="hc-debug-row" title="${e}"><span>${t}</span><b data-k="${t}">\u2014</b></div>`).join("")+'<div class="hc-debug-trace"><span>per-frame scroll</span><code data-k="trace">drag the page to record</code></div>',this.tick=this.tick.bind(this)}mount(t){t.appendChild(this.el),this.start()}start(){this.running||(this.running=!0,this.lastPaintAt=0,this.lastReportAt=0,requestAnimationFrame(this.tick))}stop(){this.running=!1}recordInference(t){this.inference.push(t)}recordTracked(){this.trackCount+=1}recordScroll(t){this.scrollCount+=1,this.scrollSteps.push(Math.abs(t))}recordCommit(t,e){this.commitReads+=1,this.commitLag.push(Math.abs(t-e)),this.lastReported!==null&&e===this.lastReported&&(this.staleReads+=1),this.lastReported=e}beginTrace(t){this.traceNode=t,this.trace=[],this.lastScrollTop=null,this.tracing=!0}endTrace(){this.tracing=!1}recordTarget(t,e){let i=t===document.scrollingElement||t===document.documentElement?"page":t.tagName.toLowerCase()+(t.id?`#${t.id}`:"");this.targetLabel=`${i} ${e?"SMOOTH":"auto"}`,this.targetSmooth=e}tick(t){if(!this.running)return;if(requestAnimationFrame(this.tick),this.lastPaintAt){let r=t-this.lastPaintAt;this.frameGaps.push(r),this.paintCount+=1,r>32&&(this.blockedCount+=1)}if(this.lastPaintAt=t,this.tracing&&this.traceNode){let r=this.traceNode.scrollTop;this.lastScrollTop!==null&&(this.trace.push(Math.round(r-this.lastScrollTop)),this.trace.length>240&&this.trace.shift()),this.lastScrollTop=r}this.lastReportAt||(this.lastReportAt=t);let e=t-this.lastReportAt;if(e<500)return;let i=r=>Math.round(r*1e3/e);this.set("paint",`${i(this.paintCount)}/s`),this.set("track",`${i(this.trackCount)}/s`),this.set("model",this.inference.values.length?`${this.inference.mean.toFixed(0)} / ${this.inference.max.toFixed(0)}ms`:"\u2014"),this.set("frame",`${this.frameGaps.percentile(.5).toFixed(0)} / ${this.frameGaps.percentile(.95).toFixed(0)}ms`),this.set("worst",`${this.frameGaps.max.toFixed(0)}ms`);let n=this.paintCount?this.blockedCount/this.paintCount*100:0;this.set("blocked",`${n.toFixed(0)}%`,n>20),this.set("scroll",this.scrollCount?`${i(this.scrollCount)}/s ${this.scrollSteps.mean.toFixed(1)}px`:"\u2014");let o=this.commitReads?this.staleReads/this.commitReads*100:0;if(this.set("commit",this.commitReads?`${this.commitLag.mean.toFixed(1)}px lag, ${o.toFixed(0)}% stale`:"\u2014",this.commitLag.mean>1),this.set("target",this.targetLabel||"\u2014",!!this.targetSmooth),this.trace.length){let r=this.trace.findIndex(a=>a!==0),h=r===-1?this.trace:this.trace.slice(r),l=this.el.querySelector('[data-k="trace"]');l&&(l.textContent=h.slice(-44).join(" "))}this.paintCount=0,this.trackCount=0,this.scrollCount=0,this.blockedCount=0,this.staleReads=0,this.commitReads=0,this.lastReportAt=t}set(t,e,i=!1){let n=this.el.querySelector(`[data-k="${t}"]`);n&&(n.textContent=e,n.classList.toggle("is-warn",i))}destroy(){this.stop(),this.el.remove()}};var Q=null;function Xt(s){return Q||(Q=import(s).catch(e=>{throw Q=null,e})),Q}async function Qt({cdn:s,numHands:t,delegate:e}){let i=await Xt(s.vision),n=await i.FilesetResolver.forVisionTasks(s.wasm),o=r=>i.HandLandmarker.createFromOptions(n,{baseOptions:{modelAssetPath:s.model,delegate:r},runningMode:"VIDEO",numHands:t,minHandDetectionConfidence:.5,minHandPresenceConfidence:.5,minTrackingConfidence:.5});try{return await o(e)}catch(r){if(e==="CPU")throw r;return o("CPU")}}async function pt(s){try{return await Qt(s)}catch(t){throw console.error("[hand-cursor] could not load the MediaPipe model",t),Object.assign(t instanceof Error?t:new Error(String(t)),{code:"model"})}}async function mt({width:s,height:t,frameRate:e}){if(!window.isSecureContext)throw Object.assign(new Error("insecure context"),{code:"insecure"});if(!navigator.mediaDevices?.getUserMedia)throw Object.assign(new Error("getUserMedia unavailable"),{code:"missing"});let i;try{i=await navigator.mediaDevices.getUserMedia({audio:!1,video:{facingMode:"user",width:{ideal:s},height:{ideal:t},frameRate:{ideal:e}}})}catch(n){let o=n?.name==="NotAllowedError"||n?.name==="SecurityError"?"denied":n?.name==="NotFoundError"||n?.name==="OverconstrainedError"?"missing":"failed";throw Object.assign(n instanceof Error?n:new Error(String(n)),{code:o})}return i}function H(s){if(s)for(let t of s.getTracks())t.stop()}var Ht=400,w=class{constructor(t={}){this.options=K(A,t),this.host=document.createElement("div"),this.host.setAttribute("data-hand-cursor",""),this.shadow=this.host.attachShadow({mode:"open"});let e=document.createElement("style");e.textContent=q,this.shadow.appendChild(e),this.panel=new S(this.options,{onToggleCamera:()=>this.running?this.stop():this.start(),onStop:()=>this.stop(),onToggleSize:()=>this.setMinimized(!this.panel.mini)}),this.panel.mount(this.shadow),this.debug=this.options.debug?new X:null,this.driver=new L(this.options,{ui:Ut(this.shadow),debug:this.debug,onEvent:(i,n)=>this.emit(i,n)}),this.driver.mount(this.panel.root),this.debug?.mount(this.panel.root),this.running=!1,this.starting=!1,this.destroyed=!1,this.stream=null,this.landmarker=null,this.frame=null,this.lastVideoTime=-1,this.lastHandAt=0,this.lastLandmarks=null,this.lastInferenceAt=0,this.applyStyleVariables(),this.onKeyDown=this.onKeyDown.bind(this),this.tick=this.tick.bind(this)}get position(){return this.driver.position}get pinching(){return this.driver.pinching}applyStyleVariables(){let{margin:t,zIndex:e,grayscale:i}=this.options;this.panel.root.style.setProperty("--hc-margin",`${t}px`),this.panel.root.style.setProperty("--hc-z",String(e)),this.panel.root.style.setProperty("--hc-video-filter",i?"grayscale(1)":"none")}mount(t=document.body){return this.mounted?this:(t.appendChild(this.host),this.mounted=!0,this.options.font&&this.injectFont(),document.addEventListener("keydown",this.onKeyDown,!0),this.options.autoStart&&this.start(),this)}injectFont(){if(document.fonts&&typeof document.fonts.check=="function"&&document.fonts.check("500 12px Inter")||document.querySelector('link[href*="family=Inter"]'))return;let e=document.createElement("link");e.rel="stylesheet",e.href=N,e.setAttribute("data-hand-cursor-font",""),document.head.appendChild(e)}onKeyDown(t){t.key==="Escape"&&this.running&&this.stop()}emit(t,e){document.dispatchEvent(new CustomEvent(`handcursor:${t}`,{detail:{...e,instance:this}}))}async start(){if(!(this.running||this.starting||this.destroyed)){this.starting=!0,this.panel.setState("loading");try{let[t,e]=await Promise.all([mt(this.options.camera),this.landmarker??pt({cdn:this.options.cdn,numHands:this.options.numHands??1,delegate:this.options.delegate})]);if(this.destroyed){H(t);return}this.stream=t,this.landmarker=e,await this.panel.attachStream(t),this.running=!0,this.starting=!1,this.lastVideoTime=-1,this.driver.reset(),this.driver.prepareHoverStyles(),this.panel.setState("live"),this.options.hideNativeCursor&&document.documentElement.style.setProperty("cursor","none","important"),this.frame=requestAnimationFrame(this.tick),this.emit("start",{})}catch(t){this.starting=!1,H(this.stream),this.stream=null;let e=this.options.strings,i=e[t?.code]||e.failed;this.panel.setState("error",i),this.emit("error",{error:t,message:i})}}}stop(){this.frame!==null&&(cancelAnimationFrame(this.frame),this.frame=null),H(this.stream),this.stream=null,this.running=!1,this.starting=!1,this.lastLandmarks=null,this.driver.release(),this.driver.reset(),this.panel.detachStream(),this.panel.setState("idle"),this.options.hideNativeCursor&&document.documentElement.style.removeProperty("cursor"),this.emit("stop",{})}setMinimized(t){this.panel.setMini(!!t),this.emit(t?"minimize":"expand",{})}destroy(){this.destroyed||(this.destroyed=!0,this.stop(),document.removeEventListener("keydown",this.onKeyDown,!0),this.landmarker?.close?.(),this.landmarker=null,this.debug?.destroy(),this.driver.destroy(),this.panel.destroy(),this.host.remove(),this.mounted=!1)}tick(t){if(!this.running)return;this.frame=requestAnimationFrame(this.tick);let e=this.panel.video;if(e.readyState<2||!e.videoWidth)return;let{maxTrackingFps:i}=this.options,n=i>0?this.lastInferenceAt+1e3/i:0;if(e.currentTime!==this.lastVideoTime&&t>=n){this.lastVideoTime=e.currentTime,this.lastInferenceAt=t;let o=this.debug?performance.now():0;try{let r=this.landmarker.detectForVideo(e,t);this.lastLandmarks=r?.landmarks?.[0]??null}catch{this.lastLandmarks=null}this.debug?.recordInference(performance.now()-o)}this.lastLandmarks?(this.lastHandAt=t,this.consumeHand(this.lastLandmarks,t,e)):t-this.lastHandAt>Ht&&this.releaseHand(),this.panel.drawHand(this.lastLandmarks,this.pinching)}consumeHand(t,e,i){return this.driver.consume(t,e,i.videoWidth/i.videoHeight)}releaseHand(){this.lastLandmarks=null,this.driver.release()}};function Ut(s){let t=null,e=i=>{t!==i&&(t?.classList.remove("hc-hover"),i?.classList.add("hc-hover"),t=i||null)};return{contains:i=>!!(i&&s.contains(i)),hover:i=>e(i?.closest?.("button")||null),tap:i=>{let n=i?.closest?.("button");n&&!n.disabled&&n.click()}}}var ft="1.0.0",g=null;function B(s={}){g?.destroy();let t=typeof location<"u"&&location.href.includes("handcursor-debug");return g=new w(t?{...s,debug:!0}:s),g.mount(s.container||document.body),g}function gt(){return g}function vt(){g?.destroy(),g=null}var jt=new Set(["autoStart","minimized","grayscale","font","hideNativeCursor"]),Gt=new Set(["margin","zIndex","numHands"]);function Kt(s){if(!s)return{};let t={};for(let[e,i]of Object.entries(s.dataset))if(e!=="manual")if(e==="model"||e==="vision"||e==="wasm")t.cdn={...t.cdn,[e]:i};else if(jt.has(e))t[e]=i!=="false";else if(Gt.has(e)){let n=Number(i);Number.isFinite(n)&&(t[e]=n)}else t[e]=i;return t}function Vt(s){if(s?.dataset.manual!==void 0)return;let t=Kt(s),e=()=>{g||B(t)};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",e,{once:!0}):e()}typeof document<"u"&&Vt(document.currentScript);var Wt={init:B,instance:gt,destroy:vt,DEFAULTS:A,VERSION:ft};return Et(Bt);})();
