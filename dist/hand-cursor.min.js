/*! hand-tracking-cursor v1.0.0 | MIT | https://github.com/willhschmid/Hand-Tracking-Cursor */
var HandCursor=(()=>{var j=Object.defineProperty;var vt=Object.getOwnPropertyDescriptor;var bt=Object.getOwnPropertyNames;var xt=Object.prototype.hasOwnProperty;var yt=(s,t)=>{for(var e in t)j(s,e,{get:t[e],enumerable:!0})},At=(s,t,e,i)=>{if(t&&typeof t=="object"||typeof t=="function")for(let n of bt(t))!xt.call(s,n)&&n!==e&&j(s,n,{get:()=>t[n],enumerable:!(i=vt(t,n))||i.enumerable});return s};var Ct=s=>At(j({},"__esModule",{value:!0}),s);var Bt={};yt(Bt,{DEFAULTS:()=>y,HandCursorController:()=>I,VERSION:()=>ft,default:()=>Vt,destroy:()=>gt,init:()=>W,instance:()=>mt});var y={position:"bottom-left",margin:16,autoStart:!1,minimized:!1,grayscale:!0,font:!0,hideNativeCursor:!1,emulateHover:!0,zIndex:2147483e3,numHands:1,debug:!1,maxTrackingFps:0,camera:{width:640,height:480,frameRate:30},region:{x:.15,y:.15},smoothing:{minCutoff:1.4,beta:.015,dCutoff:1},pinch:{on:.22,off:.32},tap:{maxDuration:800,maxTravel:32},drag:{threshold:34,holdDelay:140,holdEscape:70,mode:"write",retargetMs:70,flingScale:1,follow:.22,resample:1.35,resampleMin:24,resampleMax:90,velocityWindow:120,friction:.967,minVelocity:24,maxVelocity:3600},rotation:{enabled:!0,maxAngle:22,gain:1.8,minSpeed:.6,smoothing:.12},cursor:{scale:1,pressScale:.85},cdn:{vision:"https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@1.0.1/vision_bundle.mjs",wasm:"https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@1.0.1/wasm",model:"https://storage.googleapis.com/mediapipe-models/hand_landmarker/hand_landmarker/float16/1/hand_landmarker.task"},delegate:"GPU",strings:{intro:"Use hand tracking to control your cursor. Video never leaves your device.",enable:"Enable Camera",starting:"Starting\u2026",retry:"Try Again",minimize:"Minimize hand tracking",expand:"Expand hand tracking",disable:"Turn camera off",insecure:"Camera access needs a secure (https) connection.",denied:"Camera permission was denied. Allow access and try again.",missing:"No camera was found on this device.",model:"The hand tracking model could not load. Check your connection.",failed:"Hand tracking could not start. Please try again."}},G=s=>s!==null&&typeof s=="object"&&!Array.isArray(s);function K(s,t){let e=Array.isArray(s)?s.slice():{...s};if(!G(t))return e;for(let[i,n]of Object.entries(t))n!==void 0&&(e[i]=G(n)&&G(s?.[i])?K(s[i],n):n);return e}var l={green:"#00CA48",darkGreen:"#008630",red:"#FF4040",purple:"#FB79FF",yellow:"#FFC44F",white:"#FFFFFF",lightGray:"#F6F6F6",border:"#EBEBEB",mediumGray:"#D9D9D9",toggleGray:"#C5C5C6",iconDark:"#1C1B1F",black:"#000000",textPrimary:"rgba(0, 0, 0, 1)",textSecondary:"rgba(0, 0, 0, 0.6)",textTertiary:"rgba(0, 0, 0, 0.4)",divider:"rgba(0, 0, 0, 0.08)"},b={control:4,button:8,card:12},m={family:"Inter, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif",label:{size:12,weight:500,leading:16},body:{size:12,weight:400,leading:16}},h={panelWidth:260,panelHeight:200,miniSize:106,pad:12,gap:12,cornerInset:4,cornerButton:32,iconSize:24,ctaHeight:32,ctaPadX:12,ctaGap:8,miniCta:32,illoWidth:66,illoHeight:98},N="https://fonts.googleapis.com/css2?family=Inter:wght@400;500&display=swap";var q=`
:host {
  all: initial;
}

.hc-root {
  position: fixed;
  inset: 0;
  z-index: var(--hc-z, 2147483000);
  pointer-events: none;
  font-family: ${m.family};
  -webkit-font-smoothing: antialiased;
  color: ${l.textPrimary};
}

.hc-root * { box-sizing: border-box; }

/* ---------------------------------------------------------------- panel -- */

.hc-panel {
  position: absolute;
  pointer-events: auto;
  width: ${h.panelWidth}px;
  height: ${h.panelHeight}px;
  padding: ${h.pad}px;
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: space-between;
  background: ${l.lightGray};
  border-radius: ${b.card}px;
  overflow: hidden;
  box-shadow:
    0 0 0 1px ${l.divider},
    0 1px 2px rgba(0, 0, 0, 0.04),
    0 8px 24px rgba(0, 0, 0, 0.08);
  transition:
    width 280ms cubic-bezier(0.2, 0.8, 0.2, 1),
    height 280ms cubic-bezier(0.2, 0.8, 0.2, 1),
    background-color 200ms linear,
    opacity 200ms linear;
}

.hc-root[data-position$="-left"]  .hc-panel { left: var(--hc-margin); }
.hc-root[data-position$="-right"] .hc-panel { right: var(--hc-margin); }
.hc-root[data-position^="bottom"] .hc-panel { bottom: var(--hc-margin); }
.hc-root[data-position^="top"]    .hc-panel { top: var(--hc-margin); }

.hc-root[data-mini="true"] .hc-panel {
  width: ${h.miniSize}px;
  height: ${h.miniSize}px;
  padding: 0;
}

.hc-root[data-state="live"] .hc-panel {
  padding: 0;
  background: ${l.lightGray};
}

/* ---------------------------------------------------------------- stage -- */

.hc-stage {
  position: relative;
  /* Fixed at the illustration's size; the card distributes what is left over.
     A 98px illustration, two lines of copy and a 32px button do not leave room
     for 12px gaps inside 200px, so the gaps are what give. */
  flex: 0 0 auto;
  width: ${h.illoWidth}px;
  height: ${h.illoHeight}px;
  display: flex;
  align-items: center;
  justify-content: center;
}

.hc-root[data-state="live"] .hc-stage,
.hc-root[data-mini="true"] .hc-stage {
  position: absolute;
  inset: 0;
  width: auto;
  height: auto;
}

.hc-video,
.hc-overlay {
  position: absolute;
  inset: 0;
  width: 100%;
  height: 100%;
  display: none;
}

.hc-video {
  object-fit: cover;
  transform: scaleX(-1);
  filter: var(--hc-video-filter, none);
  /* Knocked right back so the dark icons and the skeleton stay legible over
     the #F6F6F6 card behind it. */
  opacity: var(--hc-video-opacity, 0.15);
}

.hc-root[data-state="live"] .hc-video,
.hc-root[data-state="live"] .hc-overlay { display: block; }

.hc-illo {
  display: flex;
  align-items: center;
  justify-content: center;
  width: 100%;
  height: 100%;
}

.hc-root[data-state="live"] .hc-illo { display: none; }
.hc-root[data-mini="true"] .hc-illo { padding: 10px; }

.hc-illo-img {
  display: block;
  width: 100%;
  height: 100%;
  object-fit: contain;
  user-select: none;
  -webkit-user-drag: none;
}

/* ----------------------------------------------------------------- copy -- */

.hc-copy {
  flex: none;
  margin: 0;
  max-width: 100%;
  text-align: center;
  font-size: ${m.body.size}px;
  font-weight: ${m.body.weight};
  line-height: ${m.body.leading}px;
  letter-spacing: -0.01em;
  color: ${l.textSecondary};
}

.hc-root[data-state="error"] .hc-copy { color: ${l.red}; }

.hc-root[data-state="live"] .hc-copy,
.hc-root[data-mini="true"] .hc-copy { display: none; }

/* ------------------------------------------------------------------ cta -- */

.hc-cta {
  flex: none;
  appearance: none;
  border: 0;
  margin: 0;
  display: inline-flex;
  align-items: center;
  justify-content: center;
  gap: ${h.ctaGap}px;
  height: ${h.ctaHeight}px;
  padding: 0 ${h.ctaPadX}px;
  border-radius: ${b.button}px;
  background: ${l.green};
  color: ${l.white};
  font-family: inherit;
  font-size: ${m.label.size}px;
  font-weight: ${m.label.weight};
  line-height: ${m.label.leading}px;
  letter-spacing: -0.01em;
  cursor: pointer;
  transition: background-color 150ms linear, transform 120ms ease-out;
}

.hc-cta:hover,
.hc-cta.hc-hover { background: ${l.darkGreen}; }
.hc-cta:active { transform: scale(0.97); }
.hc-cta[disabled] { opacity: 0.7; cursor: default; }

.hc-cta svg { width: ${h.iconSize}px; height: ${h.iconSize}px; flex: none; }

.hc-root[data-state="live"] .hc-cta,
.hc-root[data-mini="true"] .hc-cta { display: none; }

/* -------------------------------------------------------- corner button -- */

.hc-corner,
.hc-mini-cta {
  position: absolute;
  appearance: none;
  border: 0;
  padding: 0;
  display: none;
  align-items: center;
  justify-content: center;
  cursor: pointer;
  background: transparent;
  color: ${l.iconDark};
}

.hc-corner {
  width: ${h.cornerButton}px;
  height: ${h.cornerButton}px;
  border-radius: ${b.control}px;
  transition: background-color 150ms linear;
}

.hc-corner svg { width: ${h.iconSize}px; height: ${h.iconSize}px; }
.hc-corner:hover,
.hc-corner.hc-hover { background: ${l.divider}; }

.hc-corner--tr { top: ${h.cornerInset}px; right: ${h.cornerInset}px; display: inline-flex; }
.hc-corner--tl { top: ${h.cornerInset}px; left: ${h.cornerInset}px; }

.hc-root[data-state="live"] .hc-corner--tl { display: inline-flex; }

/* ------------------------------------------------------- minimized cta -- */

.hc-mini-cta {
  left: ${h.cornerInset}px;
  bottom: ${h.cornerInset}px;
  width: ${h.miniCta}px;
  height: ${h.miniCta}px;
  border-radius: ${b.button}px;
  background: ${l.green};
  color: ${l.white};
  transition: background-color 150ms linear, transform 120ms ease-out;
}

.hc-mini-cta svg { width: ${h.iconSize}px; height: ${h.iconSize}px; }
.hc-mini-cta:hover,
.hc-mini-cta.hc-hover { background: ${l.darkGreen}; }
.hc-mini-cta:active { transform: scale(0.94); }

.hc-root[data-mini="true"] .hc-mini-cta { display: inline-flex; }

/* Once the camera is on, the only control in the minimized card is the
   camera-off icon in the top left. The green button is the pre-enabled
   affordance and has nothing left to offer here. */
.hc-root[data-mini="true"][data-state="live"] .hc-mini-cta { display: none; }

.hc-root[data-mini="true"][data-state="loading"] .hc-mini-cta { opacity: 0.7; }

/* -------------------------------------------------------------- spinner -- */

.hc-spinner {
  width: ${h.iconSize}px;
  height: ${h.iconSize}px;
  flex: none;
  border-radius: 50%;
  border: 2px solid rgba(255, 255, 255, 0.35);
  border-top-color: ${l.white};
  animation: hc-spin 700ms linear infinite;
}

@keyframes hc-spin { to { transform: rotate(360deg); } }

/* --------------------------------------------------------------- cursor -- */

.hc-cursor {
  position: fixed;
  top: 0;
  left: 0;
  width: var(--hc-cursor-w, 20px);
  aspect-ratio: 1;
  /* The arrow tip sits at the element's origin, so rotation and the tapped
     scale both pivot on the exact point being addressed. */
  transform-origin: 0 0;
  color: ${l.black};
  opacity: 0;
  will-change: transform, opacity;
  transition: opacity 160ms linear;
}

/* display:block matters here: an inline svg sits on a text baseline and picks
   up descender space, which made the element taller than its aspect-ratio. */
.hc-cursor svg { display: block; width: 100%; height: 100%; overflow: visible; }

.hc-cursor[data-visible="true"] { opacity: 1; }

/* ---------------------------------------------------------------- debug -- */

.hc-debug {
  position: fixed;
  top: 8px;
  left: 8px;
  pointer-events: none;
  min-width: 168px;
  padding: 8px 10px;
  border-radius: ${b.button}px;
  background: rgba(0, 0, 0, 0.82);
  color: ${l.white};
  font-family: ui-monospace, SFMono-Regular, Menlo, monospace;
  font-size: 11px;
  line-height: 15px;
  font-variant-numeric: tabular-nums;
}

.hc-debug-row { display: flex; justify-content: space-between; gap: 12px; }
.hc-debug-row span { opacity: 0.55; }
.hc-debug-row b { font-weight: 500; }
.hc-debug-row b.is-warn { color: ${l.yellow}; }

.hc-debug-trace {
  margin-top: 6px;
  padding-top: 6px;
  border-top: 1px solid rgba(255, 255, 255, 0.18);
  max-width: 240px;
}

.hc-debug-trace span { display: block; opacity: 0.55; }

.hc-debug-trace code {
  display: block;
  font: inherit;
  word-spacing: 2px;
  line-height: 14px;
  overflow-wrap: anywhere;
}

/* ------------------------------------------------------------------ a11y -- */

.hc-sr {
  position: absolute;
  width: 1px;
  height: 1px;
  margin: -1px;
  padding: 0;
  overflow: hidden;
  clip: rect(0 0 0 0);
  white-space: nowrap;
  border: 0;
}

.hc-root :focus-visible {
  outline: 2px solid ${l.green};
  outline-offset: 2px;
}

@media (prefers-reduced-motion: reduce) {
  .hc-panel,
  .hc-cta,
  .hc-mini-cta,
  .hc-corner { transition-duration: 1ms; }
  .hc-spinner { animation-duration: 2s; }
}
`;var w=s=>`<svg viewBox="0 0 24 24" fill="none" aria-hidden="true" focusable="false"><path d="${s}" fill="currentColor"/></svg>`,v={videocam:w("M4 20C3.45 20 2.97917 19.8042 2.5875 19.4125C2.19583 19.0208 2 18.55 2 18V6C2 5.45 2.19583 4.97917 2.5875 4.5875C2.97917 4.19583 3.45 4 4 4H16C16.55 4 17.0208 4.19583 17.4125 4.5875C17.8042 4.97917 18 5.45 18 6V10.5L21.15 7.35C21.3167 7.18333 21.5 7.14167 21.7 7.225C21.9 7.30833 22 7.46667 22 7.7V16.3C22 16.5333 21.9 16.6917 21.7 16.775C21.5 16.8583 21.3167 16.8167 21.15 16.65L18 13.5V18C18 18.55 17.8042 19.0208 17.4125 19.4125C17.0208 19.8042 16.55 20 16 20H4ZM4 18H16V6H4V18Z"),videocamOff:w("M18.0002 10.5L21.1502 7.35001C21.3169 7.18334 21.5002 7.14167 21.7002 7.22501C21.9002 7.30834 22.0002 7.46667 22.0002 7.70001V16.3C22.0002 16.5333 21.9002 16.6917 21.7002 16.775C21.5002 16.8583 21.3169 16.8167 21.1502 16.65L18.0002 13.5C18.0002 13.7833 17.9044 14.0208 17.7127 14.2125C17.521 14.4042 17.2835 14.5 17.0002 14.5C16.7169 14.5 16.4794 14.4042 16.2877 14.2125C16.096 14.0208 16.0002 13.7833 16.0002 13.5V6.00001H9.0002C8.66686 6.00001 8.41686 5.89584 8.2502 5.68751C8.08353 5.47917 8.0002 5.25001 8.0002 5.00001C8.0002 4.75001 8.08353 4.52084 8.2502 4.31251C8.41686 4.10417 8.66686 4.00001 9.0002 4.00001H16.0002C16.5502 4.00001 17.021 4.19584 17.4127 4.58751C17.8044 4.97917 18.0002 5.45001 18.0002 6.00001V10.5ZM19.8502 22.65L1.3502 4.15001C1.16686 3.96667 1.0752 3.73334 1.0752 3.45001C1.0752 3.16667 1.16686 2.93334 1.3502 2.75001C1.53353 2.56667 1.76686 2.47501 2.0502 2.47501C2.33353 2.47501 2.56686 2.56667 2.7502 2.75001L21.2502 21.25C21.4335 21.4333 21.5252 21.6667 21.5252 21.95C21.5252 22.2333 21.4335 22.4667 21.2502 22.65C21.0669 22.8333 20.8335 22.925 20.5502 22.925C20.2669 22.925 20.0335 22.8333 19.8502 22.65ZM4.0002 4.00001L6.0002 6.00001H4.0002V18H16.0002V16L18.0002 18C18.0002 18.55 17.8044 19.0208 17.4127 19.4125C17.021 19.8042 16.5502 20 16.0002 20H4.0002C3.4502 20 2.97936 19.8042 2.5877 19.4125C2.19603 19.0208 2.0002 18.55 2.0002 18V6.00001C2.0002 5.45001 2.19603 4.97917 2.5877 4.58751C2.97936 4.19584 3.4502 4.00001 4.0002 4.00001Z"),collapse:w("M9.92658 15H6.91499C6.63057 15 6.39215 14.9042 6.19974 14.7125C6.00734 14.5208 5.91113 14.2833 5.91113 14C5.91113 13.7167 6.00734 13.4792 6.19974 13.2875C6.39215 13.0958 6.63057 13 6.91499 13H10.9304C11.2149 13 11.4533 13.0958 11.6457 13.2875C11.8381 13.4792 11.9343 13.7167 11.9343 14V18C11.9343 18.2833 11.8381 18.5208 11.6457 18.7125C11.4533 18.9042 11.2149 19 10.9304 19C10.646 19 10.4076 18.9042 10.2152 18.7125C10.0228 18.5208 9.92658 18.2833 9.92658 18V15ZM15.9497 9H18.9613C19.2458 9 19.4842 9.09583 19.6766 9.2875C19.869 9.47917 19.9652 9.71667 19.9652 10C19.9652 10.2833 19.869 10.5208 19.6766 10.7125C19.4842 10.9042 19.2458 11 18.9613 11H14.9459C14.6615 11 14.423 10.9042 14.2306 10.7125C14.0382 10.5208 13.942 10.2833 13.942 10V6C13.942 5.71667 14.0382 5.47917 14.2306 5.2875C14.423 5.09583 14.6615 5 14.9459 5C15.2303 5 15.4687 5.09583 15.6611 5.2875C15.8535 5.47917 15.9497 5.71667 15.9497 6V9Z"),expand:w("M7 17H10C10.2833 17 10.5208 17.0958 10.7125 17.2875C10.9042 17.4792 11 17.7167 11 18C11 18.2833 10.9042 18.5208 10.7125 18.7125C10.5208 18.9042 10.2833 19 10 19H6C5.71667 19 5.47917 18.9042 5.2875 18.7125C5.09583 18.5208 5 18.2833 5 18V14C5 13.7167 5.09583 13.4792 5.2875 13.2875C5.47917 13.0958 5.71667 13 6 13C6.28333 13 6.52083 13.0958 6.7125 13.2875C6.90417 13.4792 7 13.7167 7 14V17ZM17 7H14C13.7167 7 13.4792 6.90417 13.2875 6.7125C13.0958 6.52083 13 6.28333 13 6C13 5.71667 13.0958 5.47917 13.2875 5.2875C13.4792 5.09583 13.7167 5 14 5H18C18.2833 5 18.5208 5.09583 18.7125 5.2875C18.9042 5.47917 19 5.71667 19 6V10C19 10.2833 18.9042 10.5208 18.7125 10.7125C18.5208 10.9042 18.2833 11 18 11C17.7167 11 17.4792 10.9042 17.2875 10.7125C17.0958 10.5208 17 10.2833 17 10V7Z")},J='<svg viewBox="9.6 9.6 14 14" fill="none" aria-hidden="true" focusable="false"><path d="M9.40234 11.3525C8.91256 10.1281 10.1281 8.91256 11.3525 9.40234L22.6514 13.9219C23.9484 14.441 23.8937 16.2954 22.5684 16.7373L18.4326 18.1162C18.2833 18.166 18.166 18.2833 18.1162 18.4326L16.7373 22.5684C16.2954 23.8937 14.441 23.9484 13.9219 22.6514L9.40234 11.3525Z" fill="#111111" stroke="white"/></svg>';var $=[[0,1],[1,2],[2,3],[3,4],[0,5],[5,6],[6,7],[7,8],[9,10],[10,11],[11,12],[13,14],[14,15],[15,16],[0,17],[17,18],[18,19],[19,20],[5,9],[9,13],[13,17]];function _(s,t,e){let i=(s.x-t.x)*e,n=s.y-t.y;return Math.hypot(i,n)}function Et(s,t=1){return Math.max(_(s[0],s[9],t),1e-4)}function tt(s,t=1){return _(s[4],s[8],t)/Et(s,t)}function et(s){let t=s[4],e=s[8];return{x:(t.x+e.x)/2,y:(t.y+e.y)/2}}var V=3.2,It=1.6;function nt(s,t,{pinching:e=!1}={}){let i=e?l.green:l.purple;s.lineCap="round",s.lineJoin="round",s.strokeStyle=i,s.lineWidth=It,s.beginPath();for(let[r,o]of $)s.moveTo(t[r].x,t[r].y),s.lineTo(t[o].x,t[o].y);s.stroke(),s.strokeStyle=l.green,s.lineWidth=1.4,s.setLineDash(e?[]:[3,3]),s.beginPath(),s.moveTo(t[8].x,t[8].y),s.lineTo(t[4].x,t[4].y),s.stroke(),s.setLineDash([]),s.fillStyle=i;let n=V/2;for(let r of t)s.fillRect(r.x-n,r.y-n,V,V)}var rt="data:image/png;base64,";var S=class{constructor(t,e){this.options=t,this.handlers=e,this.state="idle",this.mini=!!t.minimized,this.root=document.createElement("div"),this.root.className="hc-root",this.root.dataset.position=t.position,this.root.dataset.state=this.state,this.root.dataset.mini=String(this.mini),this.root.innerHTML=`
      <div class="hc-panel" role="region" aria-label="Hand tracking trackpad">
        <div class="hc-stage">
          <video class="hc-video" playsinline muted autoplay></video>
          <canvas class="hc-overlay" aria-hidden="true"></canvas>
          <div class="hc-illo"><img class="hc-illo-img" src="${rt}" alt="" width="${h.illoWidth}" height="${h.illoHeight}" draggable="false" /></div>
        </div>
        <p class="hc-copy"></p>
        <button class="hc-cta" type="button"></button>
        <button class="hc-corner hc-corner--tl" type="button"></button>
        <button class="hc-corner hc-corner--tr" type="button"></button>
        <button class="hc-mini-cta" type="button"></button>
      </div>
      <p class="hc-sr" role="status" aria-live="polite"></p>
    `;let i=n=>this.root.querySelector(n);this.panel=i(".hc-panel"),this.video=i(".hc-video"),this.canvas=i(".hc-overlay"),this.copy=i(".hc-copy"),this.cta=i(".hc-cta"),this.cornerLeft=i(".hc-corner--tl"),this.cornerRight=i(".hc-corner--tr"),this.miniCta=i(".hc-mini-cta"),this.status=i(".hc-sr"),this.ctx=this.canvas.getContext("2d"),this.canvasSize={width:0,height:0,dpr:0},this.cornerLeft.innerHTML=v.videocamOff,this.cornerLeft.title=t.strings.disable,this.cornerLeft.setAttribute("aria-label",t.strings.disable),this.cta.addEventListener("click",n=>e.onToggleCamera(n)),this.miniCta.addEventListener("click",n=>e.onToggleCamera(n)),this.cornerLeft.addEventListener("click",()=>e.onStop()),this.cornerRight.addEventListener("click",()=>e.onToggleSize()),this.render()}mount(t){t.appendChild(this.root)}setState(t,e){this.state=t,this.root.dataset.state=t,this.message=e,this.render()}setMini(t){this.mini=t,this.root.dataset.mini=String(t),this.render()}render(){let t=this.options.strings,e=this.state==="loading",i=this.state==="live";this.copy.textContent=this.state==="error"?this.message||t.failed:t.intro;let n=e?t.starting:this.state==="error"?t.retry:t.enable;this.cta.innerHTML=(e?'<span class="hc-spinner"></span>':v.videocam)+`<span>${n}</span>`,this.cta.disabled=e,this.miniCta.innerHTML=i?v.videocamOff:v.videocam;let r=i?t.disable:t.enable;this.miniCta.title=r,this.miniCta.setAttribute("aria-label",r),this.miniCta.disabled=e;let o=this.mini?t.expand:t.minimize;this.cornerRight.innerHTML=this.mini?v.expand:v.collapse,this.cornerRight.title=o,this.cornerRight.setAttribute("aria-label",o),this.status.textContent=i?"Hand tracking is on.":this.state==="error"?this.message||t.failed:""}attachStream(t){return this.video.srcObject=t,this.video.play().catch(()=>{})}detachStream(){this.video.srcObject=null,this.clearOverlay()}syncCanvas(){let t=Math.min(window.devicePixelRatio||1,2),e=this.canvas.clientWidth,i=this.canvas.clientHeight;if(!e||!i)return null;let n=this.canvasSize;return(n.width!==e||n.height!==i||n.dpr!==t)&&(this.canvas.width=Math.round(e*t),this.canvas.height=Math.round(i*t),this.canvasSize={width:e,height:i,dpr:t}),{width:e,height:i,dpr:t}}clearOverlay(){this.canvas.width&&(this.ctx.setTransform(1,0,0,1,0,0),this.ctx.clearRect(0,0,this.canvas.width,this.canvas.height))}drawHand(t,e){let i=this.syncCanvas();if(!i)return;let{videoWidth:n,videoHeight:r}=this.video;if(this.ctx.setTransform(i.dpr,0,0,i.dpr,0,0),this.ctx.clearRect(0,0,i.width,i.height),!t||!n||!r)return;let o=Math.max(i.width/n,i.height/r),c=n*o,d=r*o,a=(i.width-c)/2,u=(i.height-d)/2,p=t.map(f=>({x:a+(1-f.x)*c,y:u+f.y*d}));nt(this.ctx,p,{pinching:e})}destroy(){this.detachStream(),this.root.remove()}};var wt=(s,t,e)=>s<t?t:s>e?e:s,F=class{constructor(t){this.options=t,this.el=document.createElement("div"),this.el.className="hc-cursor",this.el.setAttribute("aria-hidden","true"),this.el.innerHTML=J,this.x=0,this.y=0,this.angle=0,this.scale=t.cursor.scale,this.visible=!1,this.pressed=!1,this.lastMoveAt=0,this.reducedMotion=typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches}mount(t){t.appendChild(this.el)}setVisible(t){this.visible!==t&&(this.visible=t,this.el.dataset.visible=String(t))}setPressed(t){this.pressed=t}update(t,e,i,n,r){this.x=t,this.y=e;let o=this.options.rotation;if(o.enabled&&!this.reducedMotion){let d=Math.abs(i)<o.minSpeed?0:wt(i*o.gain,-o.maxAngle,o.maxAngle);this.angle+=(d-this.angle)*o.smoothing}else this.angle=0;let c=this.pressed?this.options.cursor.pressScale:this.options.cursor.scale;this.scale+=(c-this.scale)*.35,this.el.style.transform=`translate3d(${t.toFixed(2)}px, ${e.toFixed(2)}px, 0) rotate(${this.angle.toFixed(2)}deg) scale(${this.scale.toFixed(3)})`}reset(){this.angle=0,this.scale=this.options.cursor.scale,this.pressed=!1}destroy(){this.el.remove()}};var ot=/(auto|scroll|overlay)/;function ht(s){let t=s;for(;t&&t!==document.documentElement&&t!==document.body;){if(t.nodeType===1){let i=getComputedStyle(t),n=ot.test(i.overflowY)&&t.scrollHeight-t.clientHeight>1,r=ot.test(i.overflowX)&&t.scrollWidth-t.clientWidth>1;if(n||r)return{node:t,canX:r,canY:n}}t=t.parentElement||t.getRootNode()?.host||null}let e=document.scrollingElement||document.documentElement;return{node:e,canX:e.scrollWidth-e.clientWidth>1,canY:e.scrollHeight-e.clientHeight>1}}function St(s){let t=s===document.scrollingElement?document.documentElement:s,e=t.style.scrollBehavior;return t.style.setProperty("scroll-behavior","auto","important"),()=>{e?t.style.setProperty("scroll-behavior",e,""):t.style.removeProperty("scroll-behavior")}}function Ft(s){let t=s===document.scrollingElement?document.documentElement:s;return getComputedStyle(t).scrollBehavior==="smooth"}function Y(s){let t=window.devicePixelRatio||1;return Math.round(s*t)/t}function Yt({node:s,canX:t,canY:e},i,n){let r=Y(s.scrollTop-(e?n:0)),o=Y(s.scrollLeft-(t?i:0));try{s.scrollTo({top:r,left:o,behavior:"instant"})}catch{s.scrollTop=r,s.scrollLeft=o}}function ct(s){return-1/(60*Math.log(s))}function at(s,t,e){return s*ct(t)*e}var kt=.05,k=class{constructor(t,e){this.options=t,this.debug=e,this.target=null,this.askedX=0,this.askedY=0,this.appliedX=0,this.appliedY=0,this.path=[],this.interval=0,this.head=0,this.live=!1,this.velocityX=0,this.velocityY=0,this.flinging=!1,this.frame=null,this.lastFrameAt=0,this.restoreBehavior=null,this.lastRetargetAt=0,this.tick=this.tick.bind(this)}resetPath(){this.askedX=0,this.askedY=0,this.appliedX=0,this.appliedY=0,this.path=[],this.interval=0,this.head=0,this.live=!1}setTarget(t){t!==this.target&&(this.resetPath(),this.releaseBehavior()),this.target=t,t&&(this.debug?.recordTarget(t.node,Ft(t.node)),this.debug?.beginTrace(t.node),this.lastRetargetAt=0,(this.options.drag.mode==="write"||this.options.drag.mode==="hybrid")&&(this.restoreBehavior||(this.restoreBehavior=St(t.node))))}releaseBehavior(){this.restoreBehavior?.(),this.restoreBehavior=null}push(t,e,i){if(!this.target)return;this.flinging=!1,this.live=!0;let n=this.path.at(-1);if(n){let r=i-n.t;r>=8&&r<=250&&(this.interval=this.interval?this.interval*.7+r*.3:r)}if(this.path.length===0){let r=this.options.drag.resampleMax;this.path.push({t:i-r,x:this.askedX,y:this.askedY})}for(this.askedX+=t,this.askedY+=e,this.path.push({t:i,x:this.askedX,y:this.askedY});this.path.length>2&&i-this.path[0].t>1e3;)this.path.shift();this.options.drag.mode==="native"?this.retarget():this.start()}delay(){let{resample:t,resampleMin:e,resampleMax:i}=this.options.drag;return this.interval?Math.max(e,Math.min(i,this.interval*t)):i}advanceHead(t,e){let i=t-this.delay();if(!this.head)return this.head=i,this.head;this.head+=e;let n=e*kt,r=i-this.head;return this.head+=Math.max(-n,Math.min(n,r)),this.head}positionAt(t){let e=this.path;if(e.length===0)return{x:this.appliedX,y:this.appliedY};let i=e[0];if(t<=i.t)return{x:i.x,y:i.y};let n=e.at(-1);if(t>=n.t){let u=e.at(-2),p=u?n.t-u.t:0;if(!this.live||!u||p<=0)return{x:n.x,y:n.y};let f=Math.min(t-n.t,this.options.drag.resampleMax)/p;return{x:n.x+(n.x-u.x)*f,y:n.y+(n.y-u.y)*f}}let r=e.length-1;for(;r>0&&e[r-1].t>t;)r-=1;let o=e[r-1],c=e[r],d=c.t-o.t,a=d>0?(t-o.t)/d:1;return{x:o.x+(c.x-o.x)*a,y:o.y+(c.y-o.y)*a}}retarget(t=!1,e="smooth"){let i=performance.now(),{retargetMs:n}=this.options.drag;if(!t&&i-this.lastRetargetAt<n)return!0;this.lastRetargetAt=i;let{node:r,canX:o,canY:c}=this.target,d=Y(r.scrollTop-(c?this.remainingY:0)),a=Y(r.scrollLeft-(o?this.remainingX:0));try{r.scrollTo({top:d,left:a,behavior:e})}catch{return!1}return this.appliedX=this.askedX,this.appliedY=this.askedY,this.debug?.recordScroll(0),!0}fling(t,e){let{minVelocity:i,maxVelocity:n,mode:r,friction:o,flingScale:c}=this.options.drag,d=U=>Math.max(-n,Math.min(n,U)),a=d(t),u=d(e),p=Math.hypot(a,u)<i;if(this.live=!1,(r==="native"||r==="hybrid")&&(p||(this.askedX+=at(a,o,c),this.askedY+=at(u,o,c)),this.releaseBehavior(),this.retarget(!0,"smooth"))){this.stop();return}if(p)return;let f=ct(o);this.velocityX=a+this.remainingX/f,this.velocityY=u+this.remainingY/f,this.appliedX=this.askedX,this.appliedY=this.askedY,this.flinging=!0,this.start()}start(){this.frame===null&&(this.lastFrameAt=0,this.frame=requestAnimationFrame(this.tick))}stop(){this.frame!==null&&(cancelAnimationFrame(this.frame),this.frame=null),this.flinging=!1,this.resetPath(),this.velocityX=0,this.velocityY=0,this.releaseBehavior()}tick(t){if(this.frame=null,!this.target)return;let e=this.lastFrameAt?Math.min(t-this.lastFrameAt,50):1e3/60,i=e/1e3;this.lastFrameAt=t;let n=0,r=0;if(this.flinging){let{friction:c,minVelocity:d}=this.options.drag,a=c**(i*60);this.velocityX*=a,this.velocityY*=a,n=this.velocityX*i,r=this.velocityY*i,Math.hypot(this.velocityX,this.velocityY)<d&&(this.flinging=!1)}else if(this.options.drag.follow>=1)n=this.remainingX,r=this.remainingY,this.appliedX=this.askedX,this.appliedY=this.askedY;else{let c=this.positionAt(this.advanceHead(t,e));n=c.x-this.appliedX,r=c.y-this.appliedY,this.appliedX=c.x,this.appliedY=c.y}(n||r)&&(Yt(this.target,n,r),this.debug?.recordScroll(r||n));let o=Math.abs(this.remainingX)<.001&&Math.abs(this.remainingY)<.001;!this.flinging&&o?(this.releaseBehavior(),this.debug?.endTrace()):this.frame=requestAnimationFrame(this.tick)}get remainingX(){return this.askedX-this.appliedX}get remainingY(){return this.askedY-this.appliedY}};var zt="a[href], button, input, select, textarea, summary, [tabindex]",Mt=1e3;function Tt(s,t){let e=document.elementFromPoint(s,t);for(;e?.shadowRoot;){let i=e.shadowRoot.elementFromPoint(s,t);if(!i||i===e)break;e=i}return e}function z(s,t,e){return{bubbles:!0,cancelable:!0,composed:!0,view:window,clientX:s,clientY:t,screenX:(window.screenX||0)+s,screenY:(window.screenY||0)+t,...e}}var Ot={pointerId:1,pointerType:"touch",isPrimary:!0,width:1,height:1};function A(s,t,e,i,n){s.dispatchEvent(new PointerEvent(t,z(e,i,{...Ot,...n})))}function x(s,t,e,i,n){s.dispatchEvent(new MouseEvent(t,z(e,i,n)))}var M=class{constructor(t,{ui:e,onTap:i,hover:n,debug:r}={}){this.options=t,this.ui=e,this.onTap=i,this.hoverStyles=n,this.hovered=null,this.pressing=!1,this.dragging=!1,this.origin=null,this.last=null,this.samples=[],this.scrollTarget=null,this.scroller=new k(t,r)}resolve(t,e){let i=Tt(t,e);return{el:i,internal:!!this.ui?.contains(i)}}move(t,e){let{el:i,internal:n}=this.resolve(t,e);if(this.ui?.hover(n?i:null,t,e),n||!i){this.leaveHovered(t,e);return}i!==this.hovered&&(this.leaveHovered(t,e),this.hovered=i,this.hoverStyles?.set(i),A(i,"pointerover",t,e,{buttons:this.pressing?1:0}),x(i,"mouseover",t,e,{buttons:this.pressing?1:0}),i.dispatchEvent(new MouseEvent("mouseenter",{...z(t,e),bubbles:!1,cancelable:!1}))),A(i,"pointermove",t,e,{buttons:this.pressing?1:0}),x(i,"mousemove",t,e,{buttons:this.pressing?1:0})}leaveHovered(t,e){let i=this.hovered;i&&(this.hovered=null,this.hoverStyles?.clear(),A(i,"pointerout",t,e,{buttons:0}),x(i,"mouseout",t,e,{buttons:0}),i.dispatchEvent(new MouseEvent("mouseleave",{...z(t,e),bubbles:!1,cancelable:!1})))}press(t,e,i){this.scroller.stop();let{el:n,internal:r}=this.resolve(t,e);this.pressing=!0,this.dragging=!1,this.origin={x:t,y:e,t:i,el:n,internal:r},this.last={x:t,y:e,t:i},this.samples=[{x:t,y:e,t:i}],this.scrollTarget=r||!n?null:ht(n),this.scroller.setTarget(this.scrollTarget)}drag(t,e,i){if(!this.pressing)return;let n=t-this.last.x,r=e-this.last.y;this.last={x:t,y:e,t:i},this.samples.push({x:t,y:e,t:i});let o=this.options.drag.velocityWindow*2;for(;this.samples.length>2&&i-this.samples[0].t>o;)this.samples.shift();if(!this.dragging){let{threshold:c,holdDelay:d,holdEscape:a}=this.options.drag,u=Math.hypot(t-this.origin.x,e-this.origin.y);if(u<a&&i-this.origin.t<d||u<c)return;this.dragging=!0,this.leaveHovered(t,e)}this.scroller.push(n,r,i)}releaseVelocity(){let t=this.samples.at(-1);if(!t)return{x:0,y:0};let e=t.t-this.options.drag.velocityWindow,i=this.samples.length-1;for(;i>0&&this.samples[i-1].t>=e;)i-=1;let n=this.samples[Math.max(i-1,0)],r=(t.t-n.t)/Mt;return r<=0?{x:0,y:0}:{x:(t.x-n.x)/r,y:(t.y-n.y)/r}}release(t,e,i){if(!this.pressing)return;let n=this.origin;if(this.pressing=!1,this.origin=null,this.dragging){this.dragging=!1;let a=this.releaseVelocity();this.scroller.fling(a.x,a.y);return}let r=Math.hypot(t-n.x,e-n.y),o=i-n.t,{maxDuration:c,maxTravel:d}=this.options.tap;r>d||o>c||this.tap(n.x,n.y)}tap(t,e){let{el:i,internal:n}=this.resolve(t,e);if(!i)return;if(n){this.ui.tap(i,t,e),this.onTap?.({x:t,y:e,target:i,internal:!0});return}A(i,"pointerdown",t,e,{buttons:1,button:0}),x(i,"mousedown",t,e,{buttons:1,button:0});let r=i.closest?.(zt);if(r)try{r.focus({preventScroll:!0})}catch{}else document.activeElement&&document.activeElement!==document.body&&document.activeElement.blur?.();A(i,"pointerup",t,e,{buttons:0,button:0}),x(i,"mouseup",t,e,{buttons:0,button:0}),x(i,"click",t,e,{buttons:0,button:0,detail:1}),this.onTap?.({x:t,y:e,target:i,internal:!1})}cancel(t=0,e=0){this.scroller.stop(),this.pressing=!1,this.dragging=!1,this.origin=null,this.scrollTarget=null,this.ui?.hover(null,t,e),this.leaveHovered(t,e)}destroy(){this.cancel()}};function lt(s,t){return 1/(1+1/(2*Math.PI*s)/t)}var T=class{constructor(){this.value=null}filter(t,e){return this.value=this.value===null?t:e*t+(1-e)*this.value,this.value}reset(){this.value=null}},O=class{constructor({minCutoff:t=1,beta:e=0,dCutoff:i=1}={}){this.minCutoff=t,this.beta=e,this.dCutoff=i,this.x=new T,this.dx=new T}filter(t,e){e>0||(e=1/60);let i=this.x.value,n=i===null?0:(t-i)/e,r=this.dx.filter(n,lt(this.dCutoff,e)),o=this.minCutoff+this.beta*Math.abs(r);return this.x.filter(t,lt(o,e))}reset(){this.x.reset(),this.dx.reset()}},C=class{constructor(t){this.fx=new O(t),this.fy=new O(t)}filter(t,e,i){return{x:this.fx.filter(t,i),y:this.fy.filter(e,i)}}reset(){this.fx.reset(),this.fy.reset()}};var P="data-hc-hover",Pt=s=>s.replace(/:hover\b/g,`[${P}]`);function B(s,t){for(let e of s)if(e.selectorText)e.selectorText.includes(":hover")&&t.push(`${Pt(e.selectorText)}{${e.style.cssText}}`);else if(e.media){let i=[];B(e.cssRules,i),i.length&&t.push(`@media ${e.conditionText}{${i.join("")}}`)}else if(e.conditionText&&e.cssRules){let i=[];B(e.cssRules,i),i.length&&t.push(`@supports ${e.conditionText}{${i.join("")}}`)}}var R=class{constructor(){this.style=null,this.chain=[],this.sheetCount=-1,this.warned=!1,this.observer=null}build(){let t=[],e=0;for(let i of document.styleSheets)if(i.ownerNode!==this.style)try{B(i.cssRules,t)}catch{e+=1}e&&!this.warned&&(this.warned=!0,console.info(`[hand-cursor] ${e} cross-origin stylesheet(s) could not be read, so their :hover styles will not respond to the hand cursor.`)),this.style||(this.style=document.createElement("style"),this.style.setAttribute("data-hand-cursor-hover","")),this.style.textContent=t.join(`
`),document.head.appendChild(this.style),this.sheetCount=document.styleSheets.length,this.observer||this.watch()}watch(){let t=0;this.observer=new MutationObserver(()=>{document.styleSheets.length!==this.sheetCount&&(clearTimeout(t),t=setTimeout(()=>this.build(),250))}),this.observer.observe(document.documentElement,{childList:!0,subtree:!0})}set(t){if(this.chain[0]===t)return;let e=[];for(let i=t;i&&i.nodeType===1;i=i.parentElement)e.push(i);for(let i of this.chain)e.includes(i)||i.removeAttribute(P);for(let i of e)i.setAttribute(P,"");this.chain=e}clear(){for(let t of this.chain)t.removeAttribute(P);this.chain=[]}destroy(){this.clear(),this.observer?.disconnect(),this.observer=null,this.style?.remove(),this.style=null}};var dt=s=>s<0?0:s>1?1:s,L=class{constructor(t,{ui:e,onEvent:i,debug:n}={}){this.options=t,this.onEvent=i,this.debug=n,this.cursor=new F(t),this.hoverStyles=t.emulateHover?new R:null,this.touch=new M(t,{ui:e,debug:n,hover:this.hoverStyles,onTap:r=>this.emit("tap",r)}),this.filter=new C(t.smoothing),this.position=null,this.velocity={x:0,y:0},this.pinching=!1,this.lastFrameAt=0}mount(t){this.cursor.mount(t)}emit(t,e){this.onEvent?.(t,e)}consume(t,e,i=1){this.debug?.recordTracked();let n=this.lastFrameAt?(e-this.lastFrameAt)/1e3:1/60;this.lastFrameAt=e;let r=et(t),o=this.options.region,c=dt((1-r.x-o.x)/(1-2*o.x)),d=dt((r.y-o.y)/(1-2*o.y)),a=this.filter.filter(c*window.innerWidth,d*window.innerHeight,n),u=this.position;this.position=a,this.velocity=u?{x:a.x-u.x,y:a.y-u.y}:{x:0,y:0},this.cursor.setVisible(!0),this.cursor.update(a.x,a.y,this.velocity.x,this.velocity.y,e);let p=tt(t,i),{on:f,off:U}=this.options.pinch,D=this.pinching?p<U:p<f;D&&!this.pinching?(this.pinching=!0,this.cursor.setPressed(!0),this.touch.press(a.x,a.y,e),this.emit("press",{x:a.x,y:a.y})):!D&&this.pinching&&(this.pinching=!1,this.cursor.setPressed(!1),this.touch.release(a.x,a.y,e),this.emit("release",{x:a.x,y:a.y})),this.pinching?this.touch.drag(a.x,a.y,e):this.touch.move(a.x,a.y);let Z={x:a.x,y:a.y,pinching:this.pinching,ratio:p};return this.emit("move",Z),Z}release(){this.position?this.touch.cancel(this.position.x,this.position.y):this.touch.cancel(),this.pinching=!1,this.position=null,this.lastFrameAt=0,this.filter.reset(),this.cursor.setPressed(!1),this.cursor.setVisible(!1)}reset(){this.filter.reset(),this.cursor.reset(),this.lastFrameAt=0}setSmoothing(t){this.options.smoothing={...this.options.smoothing,...t},this.filter=new C(this.options.smoothing),this.lastFrameAt=0}prepareHoverStyles(){this.hoverStyles?.build()}destroy(){this.touch.destroy(),this.cursor.destroy(),this.hoverStyles?.destroy()}};var E=class{constructor(){this.values=[]}push(t){this.values.push(t),this.values.length>60&&this.values.shift()}get mean(){return this.values.length?this.values.reduce((t,e)=>t+e,0)/this.values.length:0}percentile(t){if(!this.values.length)return 0;let e=[...this.values].sort((i,n)=>i-n);return e[Math.min(e.length-1,Math.floor(e.length*t))]}get max(){return this.values.length?Math.max(...this.values):0}},Rt=[["paint","repaints per second \u2014 the rate the screen can actually update"],["track","landmark frames per second"],["model","milliseconds per inference, mean / worst"],["frame","milliseconds between repaints, median / 95th"],["worst","longest gap between repaints"],["blocked","share of repaints later than 32ms"],["scroll","scroll writes per second, and mean step"],["target","what is being scrolled, and whether its CSS asked for smooth"]],Q=class{constructor(){this.inference=new E,this.frameGaps=new E,this.scrollSteps=new E,this.paintCount=0,this.trackCount=0,this.scrollCount=0,this.blockedCount=0,this.lastPaintAt=0,this.lastReportAt=0,this.running=!1,this.targetLabel="",this.targetSmooth=!1,this.trace=[],this.traceNode=null,this.lastScrollTop=null,this.tracing=!1,this.el=document.createElement("div"),this.el.className="hc-debug",this.el.innerHTML=Rt.map(([t,e])=>`<div class="hc-debug-row" title="${e}"><span>${t}</span><b data-k="${t}">\u2014</b></div>`).join("")+'<div class="hc-debug-trace"><span>per-frame scroll</span><code data-k="trace">drag the page to record</code></div>',this.tick=this.tick.bind(this)}mount(t){t.appendChild(this.el),this.start()}start(){this.running||(this.running=!0,this.lastPaintAt=0,this.lastReportAt=0,requestAnimationFrame(this.tick))}stop(){this.running=!1}recordInference(t){this.inference.push(t)}recordTracked(){this.trackCount+=1}recordScroll(t){this.scrollCount+=1,this.scrollSteps.push(Math.abs(t))}beginTrace(t){this.traceNode=t,this.trace=[],this.lastScrollTop=null,this.tracing=!0}endTrace(){this.tracing=!1}recordTarget(t,e){let i=t===document.scrollingElement||t===document.documentElement?"page":t.tagName.toLowerCase()+(t.id?`#${t.id}`:"");this.targetLabel=`${i} ${e?"SMOOTH":"auto"}`,this.targetSmooth=e}tick(t){if(!this.running)return;if(requestAnimationFrame(this.tick),this.lastPaintAt){let r=t-this.lastPaintAt;this.frameGaps.push(r),this.paintCount+=1,r>32&&(this.blockedCount+=1)}if(this.lastPaintAt=t,this.tracing&&this.traceNode){let r=this.traceNode.scrollTop;this.lastScrollTop!==null&&(this.trace.push(Math.round(r-this.lastScrollTop)),this.trace.length>240&&this.trace.shift()),this.lastScrollTop=r}this.lastReportAt||(this.lastReportAt=t);let e=t-this.lastReportAt;if(e<500)return;let i=r=>Math.round(r*1e3/e);this.set("paint",`${i(this.paintCount)}/s`),this.set("track",`${i(this.trackCount)}/s`),this.set("model",this.inference.values.length?`${this.inference.mean.toFixed(0)} / ${this.inference.max.toFixed(0)}ms`:"\u2014"),this.set("frame",`${this.frameGaps.percentile(.5).toFixed(0)} / ${this.frameGaps.percentile(.95).toFixed(0)}ms`),this.set("worst",`${this.frameGaps.max.toFixed(0)}ms`);let n=this.paintCount?this.blockedCount/this.paintCount*100:0;if(this.set("blocked",`${n.toFixed(0)}%`,n>20),this.set("scroll",this.scrollCount?`${i(this.scrollCount)}/s ${this.scrollSteps.mean.toFixed(1)}px`:"\u2014"),this.set("target",this.targetLabel||"\u2014",!!this.targetSmooth),this.trace.length){let r=this.trace.findIndex(d=>d!==0),o=r===-1?this.trace:this.trace.slice(r),c=this.el.querySelector('[data-k="trace"]');c&&(c.textContent=o.slice(-44).join(" "))}this.paintCount=0,this.trackCount=0,this.scrollCount=0,this.blockedCount=0,this.lastReportAt=t}set(t,e,i=!1){let n=this.el.querySelector(`[data-k="${t}"]`);n&&(n.textContent=e,n.classList.toggle("is-warn",i))}destroy(){this.stop(),this.el.remove()}};var X=null;function Lt(s){return X||(X=import(s).catch(e=>{throw X=null,e})),X}async function Qt({cdn:s,numHands:t,delegate:e}){let i=await Lt(s.vision),n=await i.FilesetResolver.forVisionTasks(s.wasm),r=o=>i.HandLandmarker.createFromOptions(n,{baseOptions:{modelAssetPath:s.model,delegate:o},runningMode:"VIDEO",numHands:t,minHandDetectionConfidence:.5,minHandPresenceConfidence:.5,minTrackingConfidence:.5});try{return await r(e)}catch(o){if(e==="CPU")throw o;return r("CPU")}}async function ut(s){try{return await Qt(s)}catch(t){throw console.error("[hand-cursor] could not load the MediaPipe model",t),Object.assign(t instanceof Error?t:new Error(String(t)),{code:"model"})}}async function pt({width:s,height:t,frameRate:e}){if(!window.isSecureContext)throw Object.assign(new Error("insecure context"),{code:"insecure"});if(!navigator.mediaDevices?.getUserMedia)throw Object.assign(new Error("getUserMedia unavailable"),{code:"missing"});let i;try{i=await navigator.mediaDevices.getUserMedia({audio:!1,video:{facingMode:"user",width:{ideal:s},height:{ideal:t},frameRate:{ideal:e}}})}catch(n){let r=n?.name==="NotAllowedError"||n?.name==="SecurityError"?"denied":n?.name==="NotFoundError"||n?.name==="OverconstrainedError"?"missing":"failed";throw Object.assign(n instanceof Error?n:new Error(String(n)),{code:r})}return i}function H(s){if(s)for(let t of s.getTracks())t.stop()}var Xt=400,I=class{constructor(t={}){this.options=K(y,t),this.host=document.createElement("div"),this.host.setAttribute("data-hand-cursor",""),this.shadow=this.host.attachShadow({mode:"open"});let e=document.createElement("style");e.textContent=q,this.shadow.appendChild(e),this.panel=new S(this.options,{onToggleCamera:()=>this.running?this.stop():this.start(),onStop:()=>this.stop(),onToggleSize:()=>this.setMinimized(!this.panel.mini)}),this.panel.mount(this.shadow),this.debug=this.options.debug?new Q:null,this.driver=new L(this.options,{ui:Ht(this.shadow),debug:this.debug,onEvent:(i,n)=>this.emit(i,n)}),this.driver.mount(this.panel.root),this.debug?.mount(this.panel.root),this.running=!1,this.starting=!1,this.destroyed=!1,this.stream=null,this.landmarker=null,this.frame=null,this.lastVideoTime=-1,this.lastHandAt=0,this.lastLandmarks=null,this.lastInferenceAt=0,this.applyStyleVariables(),this.onKeyDown=this.onKeyDown.bind(this),this.tick=this.tick.bind(this)}get position(){return this.driver.position}get pinching(){return this.driver.pinching}applyStyleVariables(){let{margin:t,zIndex:e,grayscale:i}=this.options;this.panel.root.style.setProperty("--hc-margin",`${t}px`),this.panel.root.style.setProperty("--hc-z",String(e)),this.panel.root.style.setProperty("--hc-video-filter",i?"grayscale(1)":"none")}mount(t=document.body){return this.mounted?this:(t.appendChild(this.host),this.mounted=!0,this.options.font&&this.injectFont(),document.addEventListener("keydown",this.onKeyDown,!0),this.options.autoStart&&this.start(),this)}injectFont(){if(document.fonts&&typeof document.fonts.check=="function"&&document.fonts.check("500 12px Inter")||document.querySelector('link[href*="family=Inter"]'))return;let e=document.createElement("link");e.rel="stylesheet",e.href=N,e.setAttribute("data-hand-cursor-font",""),document.head.appendChild(e)}onKeyDown(t){t.key==="Escape"&&this.running&&this.stop()}emit(t,e){document.dispatchEvent(new CustomEvent(`handcursor:${t}`,{detail:{...e,instance:this}}))}async start(){if(!(this.running||this.starting||this.destroyed)){this.starting=!0,this.panel.setState("loading");try{let[t,e]=await Promise.all([pt(this.options.camera),this.landmarker??ut({cdn:this.options.cdn,numHands:this.options.numHands??1,delegate:this.options.delegate})]);if(this.destroyed){H(t);return}this.stream=t,this.landmarker=e,await this.panel.attachStream(t),this.running=!0,this.starting=!1,this.lastVideoTime=-1,this.driver.reset(),this.driver.prepareHoverStyles(),this.panel.setState("live"),this.options.hideNativeCursor&&document.documentElement.style.setProperty("cursor","none","important"),this.frame=requestAnimationFrame(this.tick),this.emit("start",{})}catch(t){this.starting=!1,H(this.stream),this.stream=null;let e=this.options.strings,i=e[t?.code]||e.failed;this.panel.setState("error",i),this.emit("error",{error:t,message:i})}}}stop(){this.frame!==null&&(cancelAnimationFrame(this.frame),this.frame=null),H(this.stream),this.stream=null,this.running=!1,this.starting=!1,this.lastLandmarks=null,this.driver.release(),this.driver.reset(),this.panel.detachStream(),this.panel.setState("idle"),this.options.hideNativeCursor&&document.documentElement.style.removeProperty("cursor"),this.emit("stop",{})}setMinimized(t){this.panel.setMini(!!t),this.emit(t?"minimize":"expand",{})}destroy(){this.destroyed||(this.destroyed=!0,this.stop(),document.removeEventListener("keydown",this.onKeyDown,!0),this.landmarker?.close?.(),this.landmarker=null,this.debug?.destroy(),this.driver.destroy(),this.panel.destroy(),this.host.remove(),this.mounted=!1)}tick(t){if(!this.running)return;this.frame=requestAnimationFrame(this.tick);let e=this.panel.video;if(e.readyState<2||!e.videoWidth)return;let{maxTrackingFps:i}=this.options,n=i>0?this.lastInferenceAt+1e3/i:0;if(e.currentTime!==this.lastVideoTime&&t>=n){this.lastVideoTime=e.currentTime,this.lastInferenceAt=t;let r=this.debug?performance.now():0;try{let o=this.landmarker.detectForVideo(e,t);this.lastLandmarks=o?.landmarks?.[0]??null}catch{this.lastLandmarks=null}this.debug?.recordInference(performance.now()-r)}this.lastLandmarks?(this.lastHandAt=t,this.consumeHand(this.lastLandmarks,t,e)):t-this.lastHandAt>Xt&&this.releaseHand(),this.panel.drawHand(this.lastLandmarks,this.pinching)}consumeHand(t,e,i){return this.driver.consume(t,e,i.videoWidth/i.videoHeight)}releaseHand(){this.lastLandmarks=null,this.driver.release()}};function Ht(s){let t=null,e=i=>{t!==i&&(t?.classList.remove("hc-hover"),i?.classList.add("hc-hover"),t=i||null)};return{contains:i=>!!(i&&s.contains(i)),hover:i=>e(i?.closest?.("button")||null),tap:i=>{let n=i?.closest?.("button");n&&!n.disabled&&n.click()}}}var ft="1.0.0",g=null;function W(s={}){g?.destroy();let t=typeof location<"u"&&location.href.includes("handcursor-debug");return g=new I(t?{...s,debug:!0}:s),g.mount(s.container||document.body),g}function mt(){return g}function gt(){g?.destroy(),g=null}var Ut=new Set(["autoStart","minimized","grayscale","font","hideNativeCursor"]),jt=new Set(["margin","zIndex","numHands"]);function Gt(s){if(!s)return{};let t={};for(let[e,i]of Object.entries(s.dataset))if(e!=="manual")if(e==="model"||e==="vision"||e==="wasm")t.cdn={...t.cdn,[e]:i};else if(Ut.has(e))t[e]=i!=="false";else if(jt.has(e)){let n=Number(i);Number.isFinite(n)&&(t[e]=n)}else t[e]=i;return t}function Kt(s){if(s?.dataset.manual!==void 0)return;let t=Gt(s),e=()=>{g||W(t)};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",e,{once:!0}):e()}typeof document<"u"&&Kt(document.currentScript);var Vt={init:W,instance:mt,destroy:gt,DEFAULTS:y,VERSION:ft};return Ct(Bt);})();
